...
// Dial codes for every country, shown in the login dropdown.
const COUNTRY_CODES = [
  ["Bangladesh", "+880"], ["India", "+91"], ["Pakistan", "+92"], ["United States", "+1"],
  ["United Kingdom", "+44"], ["Canada", "+1"], ["Australia", "+61"], ["Saudi Arabia", "+966"],
  ["United Arab Emirates", "+971"], ["Qatar", "+974"], ["Kuwait", "+965"], ["Oman", "+968"],
  ["Bahrain", "+973"], ["Malaysia", "+60"], ["Singapore", "+65"], ["Indonesia", "+62"],
  ["Nepal", "+977"], ["Sri Lanka", "+94"], ["Myanmar", "+95"], ["Thailand", "+66"],
  ["Philippines", "+63"], ["Vietnam", "+84"], ["China", "+86"], ["Japan", "+81"],
  ["South Korea", "+82"], ["Germany", "+49"], ["France", "+33"], ["Italy", "+39"],
  ["Spain", "+34"], ["Portugal", "+351"], ["Netherlands", "+31"], ["Belgium", "+32"],
  ["Switzerland", "+41"], ["Austria", "+43"], ["Sweden", "+46"], ["Norway", "+47"],
  ["Denmark", "+45"], ["Finland", "+358"], ["Poland", "+48"], ["Russia", "+7"],
  ["Turkey", "+90"], ["Greece", "+30"], ["Ireland", "+353"], ["South Africa", "+27"],
  ["Nigeria", "+234"], ["Egypt", "+20"], ["Kenya", "+254"], ["Ghana", "+233"],
  ["Brazil", "+55"], ["Mexico", "+52"], ["Argentina", "+54"], ["Colombia", "+57"],
  ["Chile", "+56"], ["Peru", "+51"], ["New Zealand", "+64"], ["Afghanistan", "+93"],
  ["Iraq", "+964"], ["Iran", "+98"], ["Israel", "+972"], ["Jordan", "+962"],
  ["Lebanon", "+961"], ["Yemen", "+967"], ["Libya", "+218"], ["Morocco", "+212"],
  ["Algeria", "+213"], ["Tunisia", "+216"], ["Sudan", "+249"], ["Ethiopia", "+251"],
  ["Tanzania", "+255"], ["Uganda", "+256"], ["Iceland", "+354"], ["Hungary", "+36"],
  ["Czech Republic", "+420"], ["Romania", "+40"], ["Bulgaria", "+359"], ["Ukraine", "+380"],
  ["Croatia", "+385"], ["Serbia", "+381"], ["Slovakia", "+421"], ["Slovenia", "+386"],
  ["Bosnia and Herzegovina", "+387"], ["Belarus", "+375"], ["Lithuania", "+370"],
  ["Latvia", "+371"], ["Estonia", "+372"], ["Kazakhstan", "+7"], ["Uzbekistan", "+998"],
  ["Azerbaijan", "+994"], ["Georgia", "+995"], ["Armenia", "+374"], ["Cambodia", "+855"],
  ["Laos", "+856"], ["Mongolia", "+976"], ["Taiwan", "+886"], ["Hong Kong", "+852"],
  ["Maldives", "+960"], ["Bhutan", "+975"], ["Brunei", "+673"], ["Fiji", "+679"],
  ["Papua New Guinea", "+675"], ["Cuba", "+53"], ["Jamaica", "+1876"], ["Venezuela", "+58"],
  ["Ecuador", "+593"], ["Bolivia", "+591"], ["Paraguay", "+595"], ["Uruguay", "+598"],
  ["Panama", "+507"], ["Costa Rica", "+506"], ["Guatemala", "+502"], ["Honduras", "+504"],
  ["El Salvador", "+503"], ["Nicaragua", "+505"], ["Dominican Republic", "+1809"],
  ["Haiti", "+509"], ["Zimbabwe", "+263"], ["Zambia", "+260"], ["Senegal", "+221"],
  ["Ivory Coast", "+225"], ["Cameroon", "+237"], ["Angola", "+244"], ["Mozambique", "+258"],
  ["Somalia", "+252"], ["Rwanda", "+250"], ["Botswana", "+267"], ["Namibia", "+264"],
  ["Malta", "+356"], ["Cyprus", "+357"], ["Luxembourg", "+352"], ["Monaco", "+377"],
  ["Andorra", "+376"], ["North Macedonia", "+389"], ["Albania", "+355"], ["Moldova", "+373"],
  ["Kyrgyzstan", "+996"], ["Tajikistan", "+992"], ["Turkmenistan", "+993"], ["Syria", "+963"],
  ["Palestine", "+970"], ["North Korea", "+850"]
];

function populateCountrySelect() {
  const select = document.getElementById('country-code-select');
  if (!select) return;
  select.innerHTML = COUNTRY_CODES
    .slice()
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([name, code]) => `<option value="${code}">${name} (${code})</option>`)
    .join('');
  // Default to Bangladesh if present
  select.value = '+880';
}
...
// Ensures exactly one RecaptchaVerifier exists, and that it is fully
// rendered (returns its widgetId) before we ever try to use it — using
// an unrendered verifier is the most common reason phone auth silently
// does nothing.
...
function resetRecaptcha() {
  if (window.recaptchaWidgetId !== undefined && window.grecaptcha) {
    try { grecaptcha.reset(window.recaptchaWidgetId); } catch (e) { /* ignore */ }
  }
}

// Cleans whatever the user typed down to plain digits, and strips a
// leading 0 (common when people type a local-format number like
// "01712345678" after already picking a country code).
...
$('#step-mobile').addEventListener('submit', async (e) => {
  e.preventDefault();
  const hint = $('#mobile-error-hint');
  hint.textContent = '';

  const raw = $('#mobile-number').value.trim();
  const digits = cleanPhoneDigits(raw);
  const countryCode = $('#country-code-select').value;

  if (!digits || digits.length < 6) {
    hint.textContent = 'Enter a valid mobile number.';
    return;
  }

  fullPhoneNumber = `${countryCode}${digits}`;
...
// Turns Firebase's raw auth error codes into something the user (and you,
// while debugging) can actually act on.
function phoneErrorMessage(err) {
  switch (err.code) {
    case 'auth/invalid-phone-number':
      return 'That phone number looks invalid. Check the country code and digits.';
    case 'auth/unauthorized-domain':
      return 'This website domain is not authorized in Firebase (Authentication → Settings → Authorized domains).';
    ...
    case 'auth/quota-exceeded':
      return 'SMS quota exceeded for this project. Check Firebase Authentication usage/billing.';
    case 'auth/operation-not-allowed':
      return 'Phone sign-in is not enabled for this Firebase project yet.';
    default:
      return err.message || 'Could not send OTP. Try again.';
  }
}
...
$('#resend-otp-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('resend OTP failed:', err.code, err.message);
    toast(phoneErrorMessage(err));
    resetRecaptcha();
  }
});
