...
/* ═══════════════════════════════════════════════════
   SAYATX — app.js  v8.0
   Persistent phone+name login · UPI payment · E-book read
   Follow fix · Messages all users · Upload resume · Logo fix
═══════════════════════════════════════════════════ */
...
function isWebView(){ const ua=navigator.userAgent||''; return /wv|WebView/i.test(ua)||!!window.gonative||!!window.median; }
...
/* Brand-X animation + colored spinner */
(function(){
...
/* Colored spinner — orange gradient arc */
.spinner{
...
/* Thumbnail section inside create post */
...
.create-thumb-section p{font-size:.78rem;font-weight:700;color:var(--tm);margin-bottom:8px;}
.create-thumb-btn{
  display:flex;align-items:center;gap:7px;padding:8px 14px;background:var(--card);
  border:1.5px solid var(--bdr2);border-radius:var(--rf);cursor:pointer;color:var(--txt);
  font-size:.82rem;font-weight:700;width:fit-content;
}
.create-thumb-preview{position:relative;display:inline-block;margin-top:8px;}
.create-thumb-preview img{width:80px;height:54px;border-radius:8px;object-fit:cover;display:block;}
.create-thumb-preview button{position:absolute;top:-6px;right:-6px;width:20px;height:20px;border-radius:50%;background:#f4212e;border:none;color:#fff;cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;}
/* Reel seek bar (touchable) */
.reel-seek-wrap{
  position:absolute;bottom:44px;left:14px;right:14px;z-index:10;
  height:20px;display:flex;align-items:center;cursor:pointer;
}
.reel-seek-track{
  width:100%;height:3px;background:rgba(255,255,255,.25);border-radius:2px;position:relative;
}
.reel-seek-fill{
  height:100%;background:linear-gradient(90deg,#f97316,#ec4899);border-radius:2px;width:0%;transition:width .1s linear;
}
.reel-seek-thumb{
...
function updateLogos(){
  qsa('.header-logo,.drawer-logo,.auth-logo-icon').forEach(el=>{
    if(el.querySelector('img[data-sx-logo]')) return;
    el.innerHTML='';
    /* White background for logo */
    el.style.background='#fff';
    el.style.padding='2px';
    const img=document.createElement('img');
    img.src=LOGO_URL; img.alt='SayatX'; img.setAttribute('data-sx-logo','1');
...
setTimeout(updateLogos,80); setTimeout(updateLogos,500);
...
function switchSection(name){
  ...
  /* Hide FAB in messages */
  const fab=$('fabPlus');
  if(fab) fab.style.display=name==='messages'?'none':'';
...
/* ══════════════════════════════════════
   AUTH — Phone+Name persistent guest login
   Same phone → same profile always
══════════════════════════════════════ */
...
function ensureUserDoc(user){
  ...
  ref.get().then(snap=>{
    ...
    const phone=localStorage.getItem('sx_phone')||'';
    const data={uid:user.uid,displayName:name,photoURL:user.photoURL||'',phone,updatedAt:TS()};
...
function updateDrawerStrip(user){
  if(!user) return;
  const name=user.displayName||localStorage.getItem('sx_name')||'User';
  setText('drawerUserName',name); setText('drawerUserHandle','@'+name.toLowerCase().replace(/[^a-z0-9]/g,'_'));
...
function refreshProfileUI(){
  ...
  setText('profileDisplayName',name); setText('profileHandle','@'+name.toLowerCase().replace(/[^a-z0-9]/g,'_'));
...
/* Build auth screen — Name + Phone → persistent guest login */
function buildAuthScreen(){
  ...
  <div style="background:var(--card);border:1px solid var(--bdr);border-radius:24px;padding:28px 22px;width:100%;max-width:380px;">
    <div style="text-align:center;margin-bottom:24px">
      <div style="width:72px;height:72px;border-radius:18px;overflow:hidden;margin:0 auto 12px;box-shadow:0 4px 20px rgba(249,115,22,.3);background:#fff;padding:3px;box-sizing:border-box">
        <img src="${LOGO_URL}" style="width:100%;height:100%;object-fit:contain;border-radius:14px;" alt="SayatX"/>
      </div>
      <h1 style="font-family:'Baloo 2',cursive;font-size:1.9rem;margin-bottom:4px"><span style="color:var(--txt)">Sayat</span><span class="brand-x">X</span></h1>
      <p style="font-size:.86rem;color:var(--tm)">Enter your name & mobile to get started</p>
    </div>
    <div style="display:flex;flex-direction:column;gap:11px">
      <div>
        <label style="font-size:.78rem;font-weight:700;color:var(--tm);display:block;margin-bottom:5px">Your Name</label>
        <input id="loginName" type="text" placeholder="e.g. Aayat" maxlength="40"
          style="width:100%;padding:12px 14px;border:1.5px solid var(--bdr2);border-radius:var(--rf);background:var(--card2);color:var(--txt);font-size:.9rem;outline:none;box-sizing:border-box"/>
      </div>
      <div>
        <label style="font-size:.78rem;font-weight:700;color:var(--tm);display:block;margin-bottom:5px">Mobile Number</label>
        <div style="display:flex;border:1.5px solid var(--bdr2);border-radius:var(--rf);overflow:hidden;background:var(--card2)">
          <span style="padding:12px 12px;font-weight:700;color:var(--tb);border-right:1.5px solid var(--bdr2);white-space:nowrap;font-size:.9rem">+91</span>
          <input id="loginPhone" type="tel" placeholder="10-digit mobile" maxlength="10" inputmode="numeric"
            style="flex:1;padding:12px 13px;background:none;border:none;outline:none;color:var(--txt);font-size:.9rem;"/>
        </div>
      </div>
      <button id="guestLoginBtn" style="width:100%;padding:14px;background:var(--pr);color:#fff;border-radius:var(--rf);font-size:.95rem;font-weight:800;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:9px;box-shadow:0 4px 14px rgba(249,115,22,.3);margin-top:4px">
        <span class="material-icons-round" style="font-size:20px">login</span>Enter SayatX
      </button>
      <p style="text-align:center;font-size:.74rem;color:var(--tm)">Same number = same profile every time</p>
    </div>
  </div>
  </div>`;

  on('guestLoginBtn','click',async()=>{
    const nameVal=($('loginName').value||'').trim();
    const phoneVal=($('loginPhone').value||'').trim();
    if(!nameVal||nameVal.length<2){toast('Enter your name (min 2 chars)','error');return;}
    if(!/^\d{10}$/.test(phoneVal)){toast('Enter valid 10-digit mobile number','error');return;}

    showLoading('Entering SayatX...');
    /* Use email/password where email=phone@sayatx.guest, password="sx_"+phone */
    const fakeEmail=phoneVal+'@sayatx.guest';
    const fakePass='sx_'+phoneVal+'_2024';

    try{
      let userCred;
      try{ userCred=await Auth.signInWithEmailAndPassword(fakeEmail,fakePass); }
      catch(e){
        if(e.code==='auth/user-not-found'||e.code==='auth/invalid-credential'||e.code==='auth/invalid-email'){
          userCred=await Auth.createUserWithEmailAndPassword(fakeEmail,fakePass);
        }else throw e;
      }
      await userCred.user.updateProfile({displayName:nameVal});
      localStorage.setItem('sx_name',nameVal);
      localStorage.setItem('sx_phone',phoneVal);
      /* Upsert user doc */
      await DB.collection('users').doc(userCred.user.uid).set({uid:userCred.user.uid,displayName:nameVal,phone:phoneVal,photoURL:'',followers:0,following:0,postCount:0,updatedAt:TS()},{merge:true});
      toast('Welcome, '+nameVal+'!','success');
      switchSection('home');
    }catch(e){ toast('Login failed: '+e.message,'error',5000); }
    finally{ hideLoading(); }
  });
  on('loginName','keydown',e=>{ if(e.key==='Enter'){$('loginPhone').focus();} });
  on('loginPhone','keydown',e=>{ if(e.key==='Enter'){$('guestLoginBtn').click();} });
}
...
/* ══════════════════════════════════════
   FEATURED — bigger, auto-play video+sound, link
══════════════════════════════════════ */
  ...
  DB.collection('featured').orderBy('createdAt','desc').limit(10).onSnapshot(snap=>{
    ...
    snap.docs.forEach(doc=>{
      ...
      if(isVideo&&d.mediaUrl){
        card.innerHTML=`<video src="${esc(d.mediaUrl)}" style="width:100%;height:100%;object-fit:cover;display:block;" autoplay loop playsinline muted></video><div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.75) 0%,transparent 50%);display:flex;flex-direction:column;justify-content:flex-end;padding:12px"><p style="color:#fff;font-size:.9rem;font-weight:800;margin-bottom:4px">Sponsored</p><span style="font-size:.7rem;color:rgba(255,255,255,.7);display:flex;align-items:center;gap:3px"><span class="material-icons-round" style="font-size:12px">star</span>Ad</span></div><button style="position:absolute;top:10px;right:10px;width:30px;height:30px;border-radius:50%;background:rgba(0,0,0,.5);border:none;display:flex;align-items:center;justify-content:center;color:#fff;cursor:pointer;" class="feat-mute"><span class="material-icons-round" style="font-size:16px">volume_off</span></button>`;
        const vid=card.querySelector('video');
        card.querySelector('.feat-mute').addEventListener('click',e=>{ e.stopPropagation(); vid.muted=!vid.muted; e.currentTarget.querySelector('.material-icons-round').textContent=vid.muted?'volume_off':'volume_up'; });
      }else{
...
function buildUserPost(post){
  ...
  const avInner=post.userPhoto?`<img src="${esc(post.userPhoto)}" alt="" onerror="this.parentNode.textContent='${name[0].toUpperCase()}'"/>`:`<span>${name[0].toUpperCase()}</span>`;
  let media='';
  if(post.mediaType==='video'&&post.mediaUrl){ media=`<div class="post-video-wrap" data-post-id="${post.id}"><video class="post-video-thumb" src="${esc(post.mediaUrl)}" preload="none" playsinline muted loop poster="${esc(post.thumbnailUrl||'')}"></video><div class="post-video-play-overlay"><div class="post-play-btn"><span class="material-icons-round">play_arrow</span></div></div></div>`; }
...
/* ══════════════════════════════════════
   REELS
══════════════════════════════════════ */
function openReels(posts,startIndex){ startIndex=startIndex||0;currentReelItems=posts; const scroll=$('reelScroll');if(!scroll) return; scroll.innerHTML=''; posts.forEach((post,i)=>{ const item=document.createElement('div');item.className='reel-item';item.dataset.i=i; const liked=!!likedPosts[post.id],name=post.userName||'User'; const av=post.userPhoto?`<img src="${esc(post.userPhoto)}" alt="" onerror="this.parentNode.textContent='${name[0].toUpperCase()}'"/>`:name[0].toUpperCase(); item.innerHTML=`<video class="reel-video" src="${esc(post.mediaUrl)}" playsinline loop preload="none" poster="${esc(post.thumbnailUrl||'')}"></video><div class="reel-grad-top"></div><div class="reel-grad-bot"></div><div style="position:absolute;top:calc(env(safe-area-inset-top,0px)+10px);left:14px;z-index:5;display:flex;align-items:center;gap:8px;"><div style="width:26px;height:26px;border-radius:7px;overflow:hidden;background:#fff;padding:2px;box-sizing:border-box"><img src="${LOGO_URL}" style="width:100%;height:100%;object-fit:contain" alt=""/></div><span style="font-family:'Baloo 2',cursive;font-weight:900;font-size:.95rem;"><span style="color:#fff">Sayat</span><span class="brand-x">X</span></span></div><div class="reel-side-btns"><div class="reel-side-btn reel-av-btn"><div class="reel-user-circle">${av}</div></div><div class="reel-side-btn reel-like-btn${liked?' reel-liked':''}" data-pid="${post.id}"><span class="material-icons-round">${liked?'favorite':'favorite_border'}</span><span class="reel-count">${fmtCount(post.likes||0)}</span></div><div class="reel-side-btn reel-cmt-btn"><span class="material-icons-round">chat_bubble_outline</span><span class="reel-count">${fmtCount(post.commentCount||0)}</span></div><div class="reel-side-btn reel-share-btn"><span class="material-icons-round">share</span><span class="reel-count">Share</span></div></div><div class="reel-info"><div class="reel-info-user"><span class="material-icons-round">account_circle</span>${esc(name)}</div><p class="reel-info-text">${esc(post.text||'')}</p></div><div class="reel-progress"><div class="reel-progress-fill"></div></div>`; const vid=item.querySelector('.reel-video'); vid.muted=false; let tt=null,tc=0; item.addEventListener('click',e=>{ if(e.target.closest('.reel-side-btns')||e.target.closest('.reel-info-user')) return; tc++;clearTimeout(tt);tt=setTimeout(()=>{ if(tc===1){if(vid.paused) vid.play().catch(()=>{}); else vid.pause();}else if(tc>=2){handleReelLike(post.id,item.querySelector('.reel-like-btn'));showHeartEffect(item);}tc=0; },280); }); item.querySelector('.reel-like-btn').addEventListener('click',e=>{e.stopPropagation();handleReelLike(post.id,e.currentTarget);}); item.querySelector('.reel-cmt-btn').addEventListener('click',e=>{e.stopPropagation();openCommentsHalf(post.id);}); item.querySelector('.reel-share-btn').addEventListener('click',e=>{e.stopPropagation();handleShare(post.id,post.text||'SayatX');}); const gp=()=>{if(post.userId){closeReels();setTimeout(()=>openUserProfile(post.userId),200);}};item.querySelector('.reel-av-btn').addEventListener('click',e=>{e.stopPropagation();gp();});item.querySelector('.reel-info-user').addEventListener('click',e=>{e.stopPropagation();gp();}); scroll.appendChild(item); }); const ov=$('reelOverlay');if(ov) ov.classList.add('reel-open'); document.body.style.overflow='hidden'; if(currentReelObs) currentReelObs.disconnect(); currentReelObs=new IntersectionObserver(entries=>{entries.forEach(entry=>{ const v=entry.target.querySelector('.reel-video'),fill=entry.target.querySelector('.reel-progress-fill'); if(entry.isIntersecting){v.play().catch(()=>{});v.ontimeupdate=()=>{if(v.duration&&fill) fill.style.width=(v.currentTime/v.duration*100)+'%';};const idx=parseInt(entry.target.dataset.i,10),p=currentReelItems[idx];if(p&&!viewedPosts.has(p.id)){viewedPosts.add(p.id);DB.collection('posts').doc(p.id).update({views:INC(1)}).catch(()=>{});}}else{v.pause();v.currentTime=0;if(fill) fill.style.width='0%';} });},{threshold:0.6}); qsa('.reel-item',scroll).forEach(it=>currentReelObs.observe(it)); if(startIndex>0) setTimeout(()=>{const items=qsa('.reel-item',scroll);if(items[startIndex]) items[startIndex].scrollIntoView({behavior:'instant'});},80); }
...
/* Comments half + Feed comments (newest first) */
...
/* ══════════════════════════════════════
   CREATE POST — thumbnail, % progress, upload survives screen off
══════════════════════════════════════ */
...
function setCreateTab(type){
  ...
  /* Inject thumbnail section if not already there */
  const sheet=$('createPostSheet');
  if(sheet&&!sheet.querySelector('.create-thumb-section')){
    const thumbSec=document.createElement('div');
    thumbSec.className='create-thumb-section';
    thumbSec.id='createThumbSection';
    thumbSec.style.display='none';
    thumbSec.innerHTML=`
      <p><span class="material-icons-round" style="font-size:14px;vertical-align:middle">image</span> Set Thumbnail (optional)</p>
      <input type="file" id="createThumbInput" accept="image/*" style="display:none"/>
      <button class="create-thumb-btn" id="createThumbBtn">
        <span class="material-icons-round" style="font-size:16px">add_photo_alternate</span>
        Choose Thumbnail Image
      </button>
      <div class="create-thumb-preview" id="createThumbPreviewWrap" style="display:none">
        <img id="createThumbPreviewImg" src="" alt="Thumbnail"/>
        <button id="createThumbRemoveBtn" title="Remove">✕</button>
      </div>`;
    /* Insert before the footer */
    const footer=sheet.querySelector('.create-post-footer,.post-footer') || sheet.lastElementChild;
    sheet.insertBefore(thumbSec, footer);
    /* Bind events */
    thumbSec.querySelector('#createThumbBtn').addEventListener('click',()=>thumbSec.querySelector('#createThumbInput').click());
    thumbSec.querySelector('#createThumbInput').addEventListener('change',e=>{
      const file=e.target.files&&e.target.files[0]; if(!file) return;
      if(file.size>5*1024*1024){toast('Max 5MB for thumbnail','error');return;}
      createThumbFile=file;
      if(createThumbBlob) URL.revokeObjectURL(createThumbBlob);
      createThumbBlob=URL.createObjectURL(file);
      const wrap=thumbSec.querySelector('#createThumbPreviewWrap'),img=thumbSec.querySelector('#createThumbPreviewImg');
      if(wrap) wrap.style.display='inline-block'; if(img) img.src=createThumbBlob;
    });
    thumbSec.querySelector('#createThumbRemoveBtn').addEventListener('click',()=>{
      if(createThumbBlob){URL.revokeObjectURL(createThumbBlob);createThumbBlob=null;} createThumbFile=null;
      const wrap=thumbSec.querySelector('#createThumbPreviewWrap');if(wrap) wrap.style.display='none';
      const inp=thumbSec.querySelector('#createThumbInput');if(inp) inp.value='';
    });
  }
  const thumbSec2=sheet?sheet.querySelector('.create-thumb-section'):null;

  if(type==='text'){
    zone.style.display='none';
    if(thumbSec2) thumbSec2.style.display='none';
    return;
  }
  zone.style.display='flex';
  if(thumbSec2) thumbSec2.style.display=['video','audio'].includes(type)?'block':'none';
...
/* ══════════════════════════════════════
   USER PROFILE — Facebook-style + follow count fix
══════════════════════════════════════ */
function buildFbPostItem(p,userInfo,onDelete){
  const el=document.createElement('div');
  el.style.cssText='background:var(--card);padding-bottom:10px;margin-bottom:10px;border-bottom:6px solid var(--card2);';
  const name=userInfo.name||'User';
  const avHtml=userInfo.photoURL?`<img src="${esc(userInfo.photoURL)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.outerHTML='${name[0].toUpperCase()}'"/>`:name[0].toUpperCase();
  if(p.status==='uploading'){ el.innerHTML=`<div style="display:flex;align-items:center;gap:10px;padding:12px 14px 10px"><div style="width:38px;height:38px;border-radius:50%;background:${randomColor(userInfo.uid)};display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;flex-shrink:0;overflow:hidden">${avHtml}</div><div><p style="font-weight:800;font-size:.88rem;color:var(--txt)">${esc(name)}</p></div></div><div style="width:100%;aspect-ratio:16/10;background:var(--card2);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px"><div class="spinner"></div><span style="font-size:.78rem;color:var(--tm);font-weight:700">Processing... ${p.uploadPct||0}%</span></div>`; return el; }
...
function openUserProfile(uid){
    ...
    newF.addEventListener('click',async()=>{
      ...
      try{
        if(isFollowing){
          ...
          await DB.collection('users').doc(uid).update({followers:INC(-1)});
          await DB.collection('users').doc(currentUser.uid).update({following:INC(-1)});
      ...
      }catch(e){ toast(e.code==='permission-denied'?'Permission denied — check Firestore rules':'Error: '+e.message,'error',5000); }
      newF.disabled=false;
    });
  }
  ...
  DB.collection('users').doc(uid).get().then(snap=>{
    ...
    DB.collection('posts').where('userId','==',uid).orderBy('createdAt','desc').limit(30).get().then(psnap=>{
      ...
      if(!posts.length){g.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round" style="font-size:44px;opacity:.3;display:block;margin-bottom:10px">grid_off</span><p>No posts yet</p></div>';return;}
...
function openYourPosts(){ if(!currentUser){toast('Login required','info');return;} const ov=$('yourPostsOverlay');if(ov) ov.classList.add('ypo-open');document.body.style.overflow='hidden';setHTML('ypoBody','<div class="pdf-loading"><div class="spinner"></div><p>Loading...</p></div>'); const userInfo={name:currentUser.displayName||'User',photoURL:currentUser.photoURL||'',uid:currentUser.uid}; DB.collection('posts').where('userId','==',currentUser.uid).orderBy('createdAt','desc').limit(50).get().then(snap=>{ const body=$('ypoBody');if(!body) return;body.innerHTML='';body.style.padding='10px 0'; if(!snap.size){body.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round" style="font-size:44px;opacity:.3;display:block">grid_off</span><p>No posts yet</p></div>';return;} snap.docs.forEach(doc=>{ const p={id:doc.id,...doc.data()}; const item=buildFbPostItem(p,userInfo,()=>openYourPosts()); body.appendChild(item); }); }).catch(err=>{setHTML('ypoBody','<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round">error</span><p>Failed: '+esc(err.message)+'</p></div>');}); }
on('ypoBackBtn','click',()=>{const ov=$('yourPostsOverlay');if(ov) ov.classList.remove('ypo-open');document.body.style.overflow='';});

function openLikedPosts(){ const ids=Object.keys(likedPosts).filter(k=>likedPosts[k]);if(!ids.length){toast('No liked posts yet','info');return;}const ov=$('yourPostsOverlay');if(!ov) return;ov.classList.add('ypo-open');document.body.style.overflow='hidden';setText('ypoTitle','Liked Posts');setHTML('ypoBody','<div class="pdf-loading"><div class="spinner"></div></div>');Promise.all(ids.slice(0,20).map(id=>DB.collection('posts').doc(id).get())).then(snaps=>{ const body=$('ypoBody');if(!body) return;body.innerHTML='';body.style.padding='10px 0'; const posts=snaps.filter(s=>s.exists&&!s.data().status).map(s=>({id:s.id,...s.data()})); if(!posts.length){body.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm)"><p>No liked posts</p></div>';return;} posts.forEach(p=>{ const ui={name:p.userName||'User',photoURL:p.userPhoto||'',uid:p.userId}; const item=buildFbPostItem(p,ui,null); body.appendChild(item); }); }).catch(()=>setHTML('ypoBody','<p style="padding:24px;color:var(--tm)">Failed</p>')); }

/* ══════════════════════════════════════
   MESSAGES — all users, search, WhatsApp style
══════════════════════════════════════ */
...
async function loadAllUsersAndConversations(){
  ...
  }catch(e){ console.error('Messages load error:',e); toast('Messages load failed: '+e.message,'error',4000); }
}
...
on('messagesSearch','input',e=>{
  ...
  if(!any&&_allUsersCache.length){
    const list=$('conversationsList');
    if(list){
      list.querySelectorAll('.conversation-item').forEach(i=>i.remove());
      _allUsersCache.filter(u=>(u.displayName||'').toLowerCase().includes(q)).forEach(u=>renderConvItem(u,null));
    }
  }
});
...
async function sendChatMessage(){ if(!currentUser||!activeChatConvId) return; const ci=$('chatInput');const text=(ci?ci.value:'').trim(); const mf=$('chatFileInput');const file=mf&&mf.files&&mf.files[0]; if(!text&&!file) return; const btn=$('chatSendBtn');if(btn) btn.disabled=true;
  ...
  try{ let mediaUrl=null,mediaType=null; if(file){const folder=file.type.startsWith('image/')?'chats/images/':'chats/videos/';mediaUrl=await uploadFile(file,folder,()=>{});mediaType=file.type.startsWith('image/')?'image':'video';if(mf) mf.value='';setStyle('chatMediaPreview','display','none');} const myName=currentUser.displayName||'User'; const convRef=DB.collection('conversations').doc(activeChatConvId); await convRef.collection('messages').add({senderId:currentUser.uid,senderName:myName,text:text||'',mediaUrl:mediaUrl||null,mediaType:mediaType||null,read:false,createdAt:TS()}); const upd={lastMessage:text||(mediaType==='image'?'📷 Image':'🎬 Video'),lastMessageAt:TS()};upd['unreadCounts.'+activeChatUid]=INC(1);await convRef.update(upd); DB.collection('notifications').add({toUid:activeChatUid,fromUid:currentUser.uid,fromName:currentUser.displayName||'User',fromPhoto:currentUser.photoURL||'',type:'message',preview:(text||(mediaType==='image'?'Sent a photo':'Sent a video')).slice(0,60),read:false,createdAt:TS()}).catch(()=>{}); if(ci){ci.value='';ci.style.height='auto';} }catch(e){toast('Failed: '+e.message,'error');} if(btn) btn.disabled=false; }
...
/* ══════════════════════════════════════
   ORDERS — e-book READ option after purchase
══════════════════════════════════════ */
...
function openOrdersOverlay(){ if(!currentUser){toast('Login to see orders','info');switchSection('profile');return;} const ov=$('ordersOverlay');if(ov) ov.classList.add('orders-open');document.body.style.overflow='hidden';setHTML('ordersBody','<div class="pdf-loading"><div class="spinner"></div><p>Loading...</p></div>');
...
/* ══════════════════════════════════════
   BUY NOW — Razorpay with UPI enabled
   Only "online" payment (no COD)
   Show product when available in "online" mode
══════════════════════════════════════ */
function handleBuyNow(product){ if(!currentUser){toast('Login to buy','info');switchSection('profile');return;} if(product.paymentMethod&&product.paymentMethod!=='online'&&product.paymentMethod!=='both'){toast('This product is not available for online purchase','info');return;} checkoutProduct=product;const isDigital=['ebook','audio'].includes(product.productType);setText('checkoutSubtitle',isDigital?'Digital product — instant access after payment.':'Enter delivery address.');setStyle('checkoutAddressFields','display',isDigital?'none':'block');/* Hide COD */const codOpt=document.querySelector('label[for="payCOD"]');if(codOpt) codOpt.style.display='none';const onlineRadio=document.querySelector('input[name="payMethod"][value="online"]');if(onlineRadio) onlineRadio.checked=true;const ov=$('checkoutModal');if(ov) ov.classList.add('modal-open');document.body.style.overflow='hidden'; }
...
function launchRazorpay(){
  ...
  const rzp=new window.Razorpay({
    key:RAZORPAY_KEY,
    amount:Math.round(checkoutProduct.price*100),
    currency:'INR',
    name:'SayatX',
    description:checkoutProduct.title,
    prefill:{
      name:currentUser.displayName||'',
      contact:(localStorage.getItem('sx_phone')||'')
    },
    theme:{color:'#f97316'},
    /* Enable UPI + all payment methods */
    method:{
      upi:1,
      card:1,
      netbanking:1,
...
async function placeOrder(method,paymentId){
  ...
  try{
    ...
    setText('successMsg',isDigital?'Your e-book is ready! Go to My Orders → Read Book.':'Order placed! We will ship it soon.');
...
on('publishFeaturedBtn','click',async()=>{
  ...
  if(!thumbF){toast('Image/Thumbnail required','error');return;}
...
console.log('%cSayatX v8.0 Ready!','color:#f97316;font-weight:900;font-size:16px');
