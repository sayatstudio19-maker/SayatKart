...
/* ═══════════════════════════════════════════════════
   SAYATX — app.js  v4.0
   WhatsApp messages · APK auth fix · Email login
   All buttons work · Reels · Admin · E2EE
═══════════════════════════════════════════════════ */
...
function isWebView(){
  ...
  return /wv|WebView/i.test(ua)||!!window.Android||(ua.includes('Version/') && ua.includes('Chrome') && !ua.includes('SamsungBrowser'));
}
...
function applyTheme(dark){
  ...
  setText('drawerThemeIcon',dark?'light_mode':'dark_mode');
...
function handleDrawerNav(nav){
  switch(nav){
    ...
    case 'notifications': openNotificationsPanel(); break;
    case 'saved':         toast('Saved — coming soon','info'); break;
    case 'settings':      if(!currentUser){toast('Login required','info');}else openSettings(); break;
    case 'about':         toast('SayatX v4.0','info'); break;
...
on('headerBrand','click',()=>{
  ...
  if(brandTaps>=10){ brandTaps=0; const pwd=prompt('Admin password:'); if(pwd===null) return; verifyAdmin(pwd).then(ok=>{ if(ok) showAdminPanel(); else toast('Wrong password','error'); }); }
});
function showAdminPanel(){ Object.values(SECTIONS).forEach(id=>{ const el=$(id); if(el) el.classList.remove('active'); }); const adm=$('sectionAdmin'); if(adm) adm.classList.add('admin-visible'); updateAdminAuthUI(); toast('Admin Panel unlocked','success'); window.scrollTo({top:0}); }
...
async function doSearch(q){
  ...
  setStyle('featuredStrip','display','none'); setStyle('feedContainer','display','none'); setStyle('searchResultsPanel','display','block');
  setText('searchResultsLabel','Results for "'+q+'"'); setStyle('searchNoResult','display','none'); setHTML('searchResultsList','');
  ...
  try{ const snap=await DB.collection('posts').orderBy('createdAt','desc').limit(100).get(); postResults=snap.docs.map(d=>({id:d.id,...d.data(),_type:'post'})).filter(p=>(p.text||'').toLowerCase().includes(lower)||(p.userName||'').toLowerCase().includes(lower)); }catch(e){}
  try{ const snap=await DB.collection('users').orderBy('displayName').get(); userResults=snap.docs.map(d=>({id:d.id,...d.data()})).filter(u=>(u.displayName||'').toLowerCase().includes(lower)).slice(0,8); }catch(e){}
  const list=$('searchResultsList'); if(!list) return;
  let found=false;
  if(userResults.length){
    found=true;
    const lbl=document.createElement('p'); lbl.style.cssText='padding:8px 14px;font-size:.76rem;font-weight:800;color:var(--tm);text-transform:uppercase;'; lbl.textContent='People'; list.appendChild(lbl);
    userResults.forEach(u=>{ const el=document.createElement('div'); el.style.cssText='display:flex;align-items:center;gap:12px;padding:11px 14px;border-bottom:1px solid var(--bdr);cursor:pointer;'; const av=document.createElement('div'); av.style.cssText='width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:900;color:#fff;font-size:1rem;flex-shrink:0;overflow:hidden;background:'+randomColor(u.id); if(u.photoURL){ const img=document.createElement('img'); img.src=u.photoURL; img.style.cssText='width:100%;height:100%;object-fit:cover'; av.appendChild(img); }else av.textContent=(u.displayName||'U')[0].toUpperCase(); el.appendChild(av); const info=document.createElement('div'); info.innerHTML='<p style="font-weight:800;font-size:.9rem;color:var(--txt)">'+esc(u.displayName||'User')+'</p><p style="font-size:.76rem;color:var(--tm)">@'+esc((u.displayName||'user').toLowerCase().replace(/\s+/g,'_'))+'</p>'; el.appendChild(info); el.addEventListener('click',()=>openUserProfile(u.id)); list.appendChild(el); });
  }
  if(postResults.length||prodResults.length){ found=true; const lbl=document.createElement('p'); lbl.style.cssText='padding:8px 14px;font-size:.76rem;font-weight:800;color:var(--tm);text-transform:uppercase;'; lbl.textContent='Posts & Products'; list.appendChild(lbl); [...postResults,...prodResults.map(p=>({...p,_type:'product'}))].forEach(item=>{ const el=item._type==='product'?buildProductPost(item):buildUserPost(item); if(el) list.appendChild(el); }); }
  if(!found){ setStyle('searchNoResult','display','block'); setText('searchNoResultMsg','No results for "'+q+'"'); }
}
...
function uploadFile(file,folder,onProgress){
  return new Promise((resolve,reject)=>{
    if(!file){reject(new Error('No file'));return;} if(!currentUser){reject(new Error('Not logged in'));return;}
    ...
    task.on('state_changed',snap=>{ const pct=snap.totalBytes>0?Math.round(snap.bytesTransferred/snap.totalBytes*100):0; if(onProgress) onProgress(pct); },err=>reject(err),()=>task.snapshot.ref.getDownloadURL().then(resolve).catch(reject));
  });
}
function generateVideoThumbnail(file){
  return new Promise(resolve=>{
    ...
    video.addEventListener('seeked',()=>{ const canvas=document.createElement('canvas'); canvas.width=video.videoWidth||640; canvas.height=video.videoHeight||360; canvas.getContext('2d').drawImage(video,0,0,canvas.width,canvas.height); canvas.toBlob(blob=>{ URL.revokeObjectURL(url); resolve(blob||null); },'image/jpeg',0.8); },{once:true});
    video.addEventListener('error',()=>{ URL.revokeObjectURL(url); resolve(null); },{once:true}); video.load();
  });
}

/* ═══════════════════════════════════════
   AUTH — WebView/APK fix + Email login
═══════════════════════════════════════ */
...
/* Handle redirect result (for WebView Google login) */
Auth.getRedirectResult().then(result=>{
  if(result&&result.user){ toast('Logged in!','success'); switchSection('home'); }
}).catch(e=>{ if(e.code&&e.code!=='auth/no-redirect-result') toast('Login error: '+e.message,'error'); });

function refreshAuthUI(user){
  const in_=!!user;
  setStyle('authScreen','display',in_?'none':'block'); setStyle('profileScreen','display',in_?'block':'none');
  setStyle('drawerLoginBtn','display',in_?'none':'flex'); setStyle('drawerLogoutBtn','display',in_?'flex':'none');
  setStyle('drawerUserStrip','display',in_?'block':'none');
  // Show/hide email login tab
  const emailTab=$('emailLoginTab'); if(emailTab) emailTab.style.display='flex';
  updateAdminAuthUI();
}
...
function ensureUserDoc(user){
  ...
  ref.get().then(snap=>{ const name=user.displayName||user.phoneNumber||user.email?.split('@')[0]||'User'; const data={uid:user.uid,displayName:name,email:user.email||'',phone:user.phoneNumber||'',photoURL:user.photoURL||'',updatedAt:TS()}; if(!snap.exists) ref.set({...data,followers:0,following:0,postCount:0,createdAt:TS()}); else ref.update(data); }).catch(()=>{});
}

function updateDrawerStrip(user){
  if(!user) return;
  const name=user.displayName||user.phoneNumber||user.email?.split('@')[0]||'User';
  setText('drawerUserName',name); setText('drawerUserHandle','@'+name.toLowerCase().replace(/\s+/g,'_'));
  ...
  DB.collection('users').doc(user.uid).get().then(snap=>{ if(!snap.exists) return; const d=snap.data(); setText('drawerPostCount',fmtCount(d.postCount||0)); setText('drawerFollowerCount',fmtCount(d.followers||0)); }).catch(()=>{});
}

function refreshProfileUI(){
  if(!currentUser) return;
  const name=currentUser.displayName||currentUser.phoneNumber||currentUser.email?.split('@')[0]||'User';
  setText('profileDisplayName',name); setText('profileHandle','@'+name.toLowerCase().replace(/\s+/g,'_'));
...
/* Google Login — popup for browser, redirect for WebView */
on('googleLoginBtn','click',()=>{
  ...
  if(isWebView()){
    ...
    Auth.signInWithPopup(provider).then(()=>{ toast('Logged in!','success'); switchSection('home'); }).catch(e=>toast('Login failed: '+e.message,'error')).finally(hideLoading);
  }
});
...
function showEmailLogin(){
  ...
  ov.innerHTML=`
    <div style="background:var(--card);border-radius:20px 20px 0 0;width:100%;max-width:480px;padding:20px 20px calc(env(safe-area-inset-bottom,0px)+20px);">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <span style="font-weight:900;font-size:1rem;color:var(--txt)">Login with Email</span>
        <button id="emailLoginClose" style="width:30px;height:30px;border-radius:50%;background:var(--card2);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--tm)"><span class="material-icons-round" style="font-size:17px">close</span></button>
      </div>
      <div style="display:flex;flex-direction:column;gap:10px">
        <input id="emailInput" type="email" placeholder="Email address" style="width:100%;padding:12px 14px;border:1.5px solid var(--bdr2);border-radius:var(--rf);background:var(--card2);color:var(--txt);font-size:.9rem;outline:none;" autocomplete="email"/>
        <input id="emailPassInput" type="password" placeholder="Password" style="width:100%;padding:12px 14px;border:1.5px solid var(--bdr2);border-radius:var(--rf);background:var(--card2);color:var(--txt);font-size:.9rem;outline:none;" autocomplete="current-password"/>
        <button id="emailSignInBtn" style="width:100%;padding:13px;background:var(--pr);color:#fff;border-radius:var(--rf);font-size:.9rem;font-weight:700;border:none;cursor:pointer;">Sign In</button>
        <button id="emailSignUpBtn" style="width:100%;padding:13px;background:var(--card2);color:var(--txt);border-radius:var(--rf);font-size:.9rem;font-weight:700;border:1.5px solid var(--bdr2);cursor:pointer;">Create Account</button>
        <button id="emailForgotBtn" style="background:none;border:none;color:var(--inf);font-size:.82rem;cursor:pointer;padding:4px;">Forgot Password?</button>
      </div>
    </div>`;
  document.body.appendChild(ov);
  ov.querySelector('#emailLoginClose').onclick=()=>ov.style.display='none';
  ov.addEventListener('click',e=>{ if(e.target===ov) ov.style.display='none'; });
  ov.querySelector('#emailSignInBtn').addEventListener('click',async()=>{
    const email=(ov.querySelector('#emailInput').value||'').trim();
    const pass=ov.querySelector('#emailPassInput').value||'';
  ...
  ov.querySelector('#emailSignUpBtn').addEventListener('click',async()=>{
    const email=(ov.querySelector('#emailInput').value||'').trim();
    const pass=ov.querySelector('#emailPassInput').value||'';
  ...
  ov.querySelector('#emailForgotBtn').addEventListener('click',async()=>{
    const email=(ov.querySelector('#emailInput').value||'').trim();
...
/* Attach email login button if exists */
...
function sendOtp(){
  ...
  const rcap=document.getElementById('recaptcha-container');
  if(rcap) rcap.innerHTML='';
  try{
    rcaptchaVerifier=new firebase.auth.RecaptchaVerifier('recaptcha-container',{
      size: isWebView()?'normal':'invisible',
      callback:()=>{}
    });
  }catch(e){ toast('reCAPTCHA error','error'); hideLoading(); return; }
  Auth.signInWithPhoneNumber('+91'+ph,rcaptchaVerifier)
    ...
    .catch(e=>{toast('OTP failed: '+e.message,'error');if(rcaptchaVerifier){try{rcaptchaVerifier.clear();}catch(e){} rcaptchaVerifier=null;}})
    .finally(hideLoading);
}
...
function openEditProfile(){
  ...
  if(existing){existing.classList.add('upo-open');document.body.style.overflow='hidden';return;}
  const ov=document.createElement('div'); ov.id='editProfileOverlay'; ov.className='user-profile-overlay upo-open'; ov.style.zIndex='615';
  ov.innerHTML=`<div class="upo-header"><button class="upo-back-btn" id="epClose"><span class="material-icons-round">close</span></button><span class="upo-header-title">Edit Profile</span></div>
  <div class="upo-body" style="padding:20px"><div style="margin-bottom:14px"><label style="display:block;font-size:.8rem;font-weight:700;color:var(--txt);margin-bottom:6px">Display Name</label>
  <input id="editNameInput" type="text" value="${esc(currentUser.displayName||currentUser.email?.split('@')[0]||'')}" style="width:100%;padding:11px 13px;border:1.5px solid var(--bdr2);border-radius:var(--rf);background:var(--card2);color:var(--txt);font-size:.9rem;outline:none;" placeholder="Your display name" maxlength="50"/></div>
  <button id="saveProfileBtn" style="display:flex;align-items:center;justify-content:center;gap:7px;width:100%;padding:13px;background:var(--pr);color:#fff;border-radius:var(--rf);font-size:.9rem;font-weight:700;border:none;cursor:pointer"><span class="material-icons-round">save</span>Save Changes</button></div>`;
  document.body.appendChild(ov); document.body.style.overflow='hidden';
  ov.querySelector('#epClose').onclick=()=>{ov.classList.remove('upo-open');document.body.style.overflow='';};
  ov.querySelector('#saveProfileBtn').addEventListener('click',async()=>{
    ...
    showLoading('Saving...');
    try{ await currentUser.updateProfile({displayName:newName}); await DB.collection('users').doc(currentUser.uid).update({displayName:newName,updatedAt:TS()}); refreshProfileUI(); updateDrawerStrip(currentUser); ov.classList.remove('upo-open'); document.body.style.overflow=''; toast('Profile updated!','success'); }
...
function openSettings(){
  ...
  if(existing){existing.classList.add('upo-open');document.body.style.overflow='hidden';return;}
  const ov=document.createElement('div'); ov.id='settingsOverlay'; ov.className='user-profile-overlay upo-open'; ov.style.zIndex='615';
  ov.innerHTML=`<div class="upo-header"><button class="upo-back-btn" id="settClose"><span class="material-icons-round">close</span></button><span class="upo-header-title">Settings</span></div>
  <div class="upo-body"><div style="padding:14px"><div style="background:var(--card2);border-radius:var(--r2);border:1px solid var(--bdr);overflow:hidden">
    ...
    <button id="settPhotoBtn" style="display:flex;align-items:center;gap:12px;width:100%;padding:14px;text-align:left;background:none;cursor:pointer;color:var(--txt)"><span class="material-icons-round" style="color:var(--pr)">photo_camera</span><span style="font-weight:700">Change Profile Photo</span></button>
  </div><button onclick="handleLogout()" style="display:flex;align-items:center;gap:10px;width:100%;margin-top:12px;padding:13px;border-radius:var(--rf);background:var(--er-b);border:1px solid rgba(244,33,46,.2);color:var(--er);font-weight:700;cursor:pointer"><span class="material-icons-round">logout</span>Logout</button></div></div>`;
  document.body.appendChild(ov); document.body.style.overflow='hidden';
  ov.querySelector('#settClose').onclick=()=>{ov.classList.remove('upo-open');document.body.style.overflow='';};
  ov.querySelector('#settEditBtn').onclick=()=>{ov.classList.remove('upo-open');document.body.style.overflow='';openEditProfile();};
  ov.querySelector('#settPhotoBtn').onclick=()=>{ov.classList.remove('upo-open');document.body.style.overflow='';const fi=$('avatarFileInput');if(fi) fi.click();};
}
...
function listenNotifications(uid){
  ...
  notifUnsub=DB.collection('notifications').where('toUid','==',uid).where('read','==',false)
    .onSnapshot(snap=>{ const n=snap.size; const dot=$('headerNotifDot'); if(dot){dot.style.display=n?'block':'none';dot.style.background='var(--er)';} setStyle('drawerNotifBadge','display',n?'inline-flex':'none'); setText('drawerNotifBadge',String(n)); },()=>{});
}

function openNotificationsPanel(){
  if(!currentUser){toast('Login to see notifications','info');return;}
  const existing=document.getElementById('notifOverlay');
  if(existing){existing.classList.add('upo-open');document.body.style.overflow='hidden';loadNotifications();return;}
  const ov=document.createElement('div'); ov.id='notifOverlay'; ov.className='user-profile-overlay upo-open'; ov.style.zIndex='615';
  ov.innerHTML=`<div class="upo-header"><button class="upo-back-btn" id="notifClose"><span class="material-icons-round">close</span></button><span class="upo-header-title">Notifications</span></div><div class="upo-body" id="notifBody"><div class="pdf-loading"><div class="spinner"></div></div></div>`;
  document.body.appendChild(ov); document.body.style.overflow='hidden';
  ov.querySelector('#notifClose').onclick=()=>{ov.classList.remove('upo-open');document.body.style.overflow='';};
  loadNotifications();
}
async function loadNotifications(){
  ...
  try{
    ...
    body.innerHTML='';
    if(!snap.size){body.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round" style="font-size:44px;opacity:.3;display:block;margin-bottom:12px">notifications_none</span><p>No notifications</p></div>';return;}
    const batch=DB.batch(); snap.docs.forEach(d=>{if(!d.data().read) batch.update(d.ref,{read:true});}); batch.commit().catch(()=>{});
    snap.docs.forEach(doc=>{
      const n=doc.data();
      const el=document.createElement('div'); el.style.cssText='display:flex;align-items:center;gap:12px;padding:13px 16px;border-bottom:1px solid var(--bdr);cursor:pointer;';
      const av=document.createElement('div'); av.style.cssText='width:40px;height:40px;border-radius:50%;background:'+randomColor(n.fromUid)+';display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:.95rem;flex-shrink:0;'; av.textContent=(n.fromName||'U')[0].toUpperCase();
      const typeText=n.type==='follow'?'followed you':n.type==='like'?'liked your post':n.type==='comment'?'commented on your post':n.type==='message'?'sent you a message':'interacted with you';
      const info=document.createElement('div'); info.innerHTML='<p style="font-weight:700;font-size:.88rem;color:var(--txt)">'+esc(n.fromName||'User')+'</p><p style="font-size:.78rem;color:var(--tm)">'+typeText+' · '+fmtAgo(n.createdAt)+'</p>';
      el.appendChild(av); el.appendChild(info);
      el.addEventListener('click',()=>{
        const notifOv=document.getElementById('notifOverlay'); if(notifOv){notifOv.classList.remove('upo-open');} document.body.style.overflow='';
        if(n.type==='message'){switchSection('messages');setTimeout(()=>openChatWith(n.fromUid),300);}
        else if(n.type==='follow') openUserProfile(n.fromUid);
      });
      body.appendChild(el);
    });
  }catch(e){body.innerHTML='<p style="padding:20px;color:var(--tm)">Failed to load</p>';}
}
...
function handleDeepLink(){
  ...
  if(postMatch){ DB.collection('posts').doc(postMatch[1]).get().then(snap=>{ if(!snap.exists) return; const post={id:snap.id,...snap.data()}; if(post.mediaType==='video'&&post.mediaUrl) openReels([post],0); else if(post.mediaType==='photo'&&post.mediaUrl) openPhotoViewer(post.mediaUrl,post.text||'',post); }).catch(()=>{}); }
  ...
  DB.collection('featured').orderBy('createdAt','desc').limit(10)
    .onSnapshot(snap=>{
      ...
      snap.docs.forEach(doc=>{
        ...
        const isVideo=d.mediaType==='video'||d.type==='video', isAudio=d.mediaType==='audio'||d.type==='audio';
        const icon=isVideo?'play_circle':isAudio?'headphones':'image', label=isVideo?'Video':isAudio?'Audio':'Image';
        const card=document.createElement('div'); card.className='featured-card';
        card.innerHTML=(d.thumbnailUrl?`<img class="featured-card-thumb" src="${esc(d.thumbnailUrl)}" loading="lazy" onerror="this.style.display='none'"/>`:'')
        ...
        card.addEventListener('click',()=>{
          if(isVideo&&d.mediaUrl) openReels([{id:d.id,mediaUrl:d.mediaUrl,mediaType:'video',text:d.title||'',userName:'SayatX',userPhoto:LOGO_URL,thumbnailUrl:d.thumbnailUrl||'',likes:0,commentCount:0,views:0,createdAt:d.createdAt}],0);
...
═══════════════════════════════════════ */
let feedFilter='all';
function loadFeed(filter){ feedFilter=filter||'all'; if(feedUnsub){feedUnsub();feedUnsub=null;} const skel=$('feedSkeleton'); if(skel) skel.style.display='block'; closeSearchPanel(); clearFeedPosts();
  ...
  feedUnsub=query.onSnapshot(snap=>{ if(skel) skel.style.display='none'; const posts=snap.docs.map(d=>({id:d.id,_type:'post',...d.data()})); let items=[...posts]; if(filter==='all'){const prods=allProducts.map(p=>({...p,_type:'product'}));items=mergeByDate([...posts,...prods]);} window._feedItems=items; renderFeed(items); },()=>{ if(skel) skel.style.display='none'; });
}
...
function buildUserPost(post){
  ...
  else if(post.mediaType==='audio'&&post.mediaUrl){ media=`<div class="post-audio-card"><div class="post-audio-play-btn"><span class="material-icons-round">play_arrow</span></div><div class="post-audio-info"><p class="post-audio-title">${esc(post.text||'Audio')}</p><div class="post-audio-bar"><div class="post-audio-bar-fill"></div></div><div class="post-audio-times"><span>0:00</span><span>—</span></div></div><span class="post-audio-type-tag">Audio</span></div>`; }
  ...
  const videoWrap=el.querySelector('.post-video-wrap');
  if(videoWrap){
    let tapTimer=null,tapCount=0;
    videoWrap.addEventListener('click',()=>{ tapCount++; clearTimeout(tapTimer); tapTimer=setTimeout(()=>{ if(tapCount===1){const vids=(window._feedItems||[]).filter(i=>i._type==='post'&&i.mediaType==='video'&&i.mediaUrl);const idx=vids.findIndex(p=>p.id===post.id);openReels(vids.length?vids:[post],Math.max(0,idx));DB.collection('posts').doc(post.id).update({views:INC(1)}).catch(()=>{}); }else if(tapCount>=2){handleLike(post.id,el.querySelector('.post-like-btn'),el.querySelector('.like-count'));showHeartEffect(videoWrap);} tapCount=0; },300); });
  }
...
function openPhotoViewer(url,caption,post){
  ...
  if(!ov){ ov=document.createElement('div'); ov.id='photoViewerOverlay'; ov.style.cssText='position:fixed;inset:0;background:#000;z-index:620;display:flex;flex-direction:column;align-items:center;justify-content:center;'; ov.innerHTML=`<div style="position:absolute;top:calc(env(safe-area-inset-top,0px)+10px);left:0;right:0;display:flex;align-items:center;justify-content:space-between;padding:0 14px;z-index:2"><button id="pvClose" style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.15);display:flex;align-items:center;justify-content:center;color:#fff;border:none;cursor:pointer"><span class="material-icons-round">close</span></button><button id="pvShare" style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.15);display:flex;align-items:center;justify-content:center;color:#fff;border:none;cursor:pointer"><span class="material-icons-round">share</span></button></div><img id="pvImg" style="max-width:100%;max-height:88dvh;object-fit:contain;display:block;"/><p id="pvCaption" style="color:rgba(255,255,255,.8);font-size:.85rem;padding:10px 14px;text-align:center;max-width:600px;"></p>`; document.body.appendChild(ov); ov.querySelector('#pvClose').onclick=()=>{ov.style.display='none';document.body.style.overflow='';document.body.style.overflow='';}; }
  ov.querySelector('#pvImg').src=url; ov.querySelector('#pvCaption').textContent=caption||'';
  ov.querySelector('#pvShare').onclick=()=>{ if(post) handleShare(post.id,caption||'SayatX'); else navigator.clipboard.writeText(url).then(()=>toast('Copied!','success')).catch(()=>{}); };
...
function openReels(posts,startIndex){
  ...
  posts.forEach((post,i)=>{
    ...
    item.innerHTML=`<video class="reel-video" src="${esc(post.mediaUrl)}" playsinline loop preload="none" poster="${esc(post.thumbnailUrl||'')}"></video>
    ...
    item.addEventListener('click',e=>{
      ...
      tapTimer=setTimeout(()=>{ if(tapCount===1){if(vid.paused) vid.play().catch(()=>{}); else vid.pause();}else if(tapCount>=2){handleReelLike(post.id,item.querySelector('.reel-like-btn'));showHeartEffect(item);} tapCount=0; },300);
    });
  ...
  currentReelObs=new IntersectionObserver(entries=>{
    entries.forEach(entry=>{ const v=entry.target.querySelector('.reel-video'),fill=entry.target.querySelector('.reel-progress-fill');
...
function openCommentsHalf(postId){
  ...
  let ov=document.getElementById('commentsHalfOverlay');
  if(!ov){
    ov=document.createElement('div'); ov.id='commentsHalfOverlay'; ov.style.cssText='position:fixed;inset:0;z-index:700;display:flex;flex-direction:column;justify-content:flex-end;';
    ov.innerHTML=`<div style="position:absolute;inset:0;background:rgba(0,0,0,.4);" id="cHalfBg"></div>
      <div style="position:relative;z-index:1;background:var(--card);border-radius:20px 20px 0 0;height:55dvh;display:flex;flex-direction:column;padding-bottom:env(safe-area-inset-bottom,0px);">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid var(--bdr);flex-shrink:0;"><span style="font-weight:800;font-size:.95rem;color:var(--txt)">Comments</span><button id="cHalfClose" style="width:30px;height:30px;border-radius:50%;background:var(--card2);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;"><span class="material-icons-round" style="font-size:17px;color:var(--tm)">close</span></button></div>
        <div id="cHalfList" style="flex:1;overflow-y:auto;padding:8px 0;"></div>
        <div style="padding:8px 12px;border-top:1px solid var(--bdr);display:flex;gap:8px;align-items:center;flex-shrink:0;">
          <div style="flex:1;background:var(--card2);border:1px solid var(--bdr2);border-radius:var(--rf);padding:8px 13px;"><input type="text" id="cHalfInput" placeholder="Add a comment..." style="width:100%;background:none;border:none;outline:none;color:var(--txt);font-size:.86rem;" maxlength="500"/></div>
          <button id="cHalfSend" style="width:36px;height:36px;border-radius:50%;background:var(--pr);color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;"><span class="material-icons-round" style="font-size:17px">send</span></button>
        </div>
      </div>`;
    document.body.appendChild(ov);
    ov.querySelector('#cHalfBg').onclick=()=>{ov.style.display='none';};
    ov.querySelector('#cHalfClose').onclick=()=>{ov.style.display='none';};
    ov.querySelector('#cHalfSend').addEventListener('click',sendHalfComment);
    ov.querySelector('#cHalfInput').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();sendHalfComment();}});
  }
  ov.style.display='flex'; loadHalfComments(postId);
}
...
on('createSubmitBtn','click',async()=>{
  ...
  try{
    ...
    if(file){
      ...
      if(createPostType==='video'){ try{ const thumbBlob=await generateVideoThumbnail(file); if(thumbBlob){ const tf=new File([thumbBlob],'thumb.jpg',{type:'image/jpeg'}); thumbnailUrl=await uploadFile(tf,'posts/thumbs/',()=>{}); } }catch(e){} }
      mediaUrl=await uploadFile(file,folder,()=>{});
      if(createPostType==='photo') thumbnailUrl=mediaUrl;
    }
    await DB.collection('posts').add({ userId:currentUser.uid,userName:name,userPhoto:currentUser.photoURL||'',text:text||'',mediaType:file?createPostType:'text',mediaUrl:mediaUrl||null,thumbnailUrl:thumbnailUrl||null,likes:0,commentCount:0,views:0,createdAt:TS() });
    await DB.collection('users').doc(currentUser.uid).update({postCount:INC(1)}).catch(()=>{});
    toast('Post published!','success');
  }catch(err){ if(err.code==='storage/unauthorized') toast('Storage permission denied. Check Firebase rules.','error',6000); else toast('Post failed: '+(err.message||err.code),'error',5000); }
...
function openUserProfile(uid){
  ...
  if(currentUser&&!isOwn){ DB.collection('follows').doc(currentUser.uid+'_'+uid).get().then(snap=>{ isFollowing=snap.exists; if(followBtn){followBtn.classList.toggle('btn-following',isFollowing);followBtn.innerHTML=isFollowing?'<span class="material-icons-round">person_remove</span> Following':'<span class="material-icons-round">person_add</span> Follow';} }).catch(()=>{}); }
  const newF=followBtn?followBtn.cloneNode(true):null;
  if(newF&&followBtn){ followBtn.parentNode.replaceChild(newF,followBtn); newF.addEventListener('click',async()=>{ if(!currentUser){toast('Login to follow','info');return;} showLoading(isFollowing?'Unfollowing...':'Following...'); const docId=currentUser.uid+'_'+uid; try{ if(isFollowing){await DB.collection('follows').doc(docId).delete();await Promise.all([DB.collection('users').doc(uid).update({followers:INC(-1)}),DB.collection('users').doc(currentUser.uid).update({following:INC(-1)})]);isFollowing=false;newF.classList.remove('btn-following');newF.innerHTML='<span class="material-icons-round">person_add</span> Follow';toast('Unfollowed','info');}else{await DB.collection('follows').doc(docId).set({followerUid:currentUser.uid,followingUid:uid,createdAt:TS()});await Promise.all([DB.collection('users').doc(uid).update({followers:INC(1)}),DB.collection('users').doc(currentUser.uid).update({following:INC(1)})]);isFollowing=true;newF.classList.add('btn-following');newF.innerHTML='<span class="material-icons-round">person_remove</span> Following';toast('Following!','success');const myName=currentUser.displayName||currentUser.phoneNumber||'User';DB.collection('notifications').add({toUid:uid,fromUid:currentUser.uid,fromName:myName,type:'follow',read:false,createdAt:TS()}).catch(()=>{});} }catch(e){toast('Error: '+e.message,'error');}hideLoading(); }); }
  ...
  /* Facebook style grid */
  DB.collection('posts').where('userId','==',uid).orderBy('createdAt','desc').limit(30).get().then(snap=>{ const grid=$('upoPostsGrid');if(!grid) return; grid.innerHTML=''; if(!snap.size){grid.innerHTML='<div class="upo-grid-empty"><span class="material-icons-round">grid_off</span><p>No posts yet</p></div>';return;} snap.docs.forEach(doc=>{ const p={id:doc.id,...doc.data()}; const item=document.createElement('div'); item.className='upo-grid-item'; const isVid=p.mediaType==='video',thumb=p.thumbnailUrl||p.mediaUrl||''; if(thumb){item.innerHTML=isVid?`<video src="${esc(thumb)}" preload="none" muted playsinline></video><span class="material-icons-round upo-grid-play">play_circle</span>`:`<img src="${esc(thumb)}" loading="lazy"/>`;item.addEventListener('click',()=>{closeUpo();if(isVid) openReels([p],0);else openPhotoViewer(p.mediaUrl||thumb,p.text||'',p);});}else{item.style.background='var(--pr-g)';item.innerHTML='<span class="material-icons-round" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:var(--pr)">notes</span>';} grid.appendChild(item); }); }).catch(()=>{});
}
...
function openYourPosts(){ if(!currentUser){toast('Login required','info');return;} const ov=$('yourPostsOverlay');if(ov) ov.classList.add('ypo-open'); document.body.style.overflow='hidden'; setHTML('ypoBody','<div class="pdf-loading"><div class="spinner"></div><p>Loading your posts...</p></div>');
  DB.collection('posts').where('userId','==',currentUser.uid).orderBy('createdAt','desc').limit(50).get().then(snap=>{ const body=$('ypoBody');if(!body) return; body.innerHTML=''; if(!snap.size){body.innerHTML='<div class="upo-grid-empty" style="grid-column:unset"><span class="material-icons-round">grid_off</span><p>No posts yet</p></div>';return;} snap.docs.forEach(doc=>{ const p={id:doc.id,...doc.data()}; const el=document.createElement('div');el.className='ypo-post-item'; const thumb=p.thumbnailUrl||p.mediaUrl||''; const tHtml=(p.mediaType==='video'||p.mediaType==='photo')&&thumb?`<div class="ypo-thumb"><img src="${esc(thumb)}" onerror="this.style.display='none'"/></div>`:p.mediaType==='audio'?'<div class="ypo-thumb"><span class="material-icons-round">headphones</span></div>':'<div class="ypo-thumb"><span class="material-icons-round">notes</span></div>'; el.innerHTML=tHtml+`<div class="ypo-info"><p class="ypo-post-text">${esc(p.text||'(No text)')}</p><div class="ypo-stats-row"><span class="ypo-stat"><span class="material-icons-round">favorite</span>${fmtCount(p.likes||0)} likes</span><span class="ypo-stat"><span class="material-icons-round">visibility</span>${fmtCount(p.views||0)} views</span><span class="ypo-stat"><span class="material-icons-round">comment</span>${fmtCount(p.commentCount||0)}</span></div></div>`; el.style.cursor='pointer'; el.addEventListener('click',()=>{if(p.mediaType==='video') openReels([p],0);else if(p.mediaType==='photo') openPhotoViewer(p.mediaUrl,p.text||'',p);}); body.appendChild(el); }); }).catch(err=>{setHTML('ypoBody','<div class="upo-grid-empty" style="grid-column:unset"><span class="material-icons-round">error</span><p>Failed to load</p><small>'+esc(err.message)+'</small></div>');}); }
...
/* ═══════════════════════════════════════
   MESSAGES — WhatsApp style ALL users
═══════════════════════════════════════ */
...
async function loadAllUsersAndConversations(){
  ...
  const existingItems=list.querySelectorAll('.conversation-item'); existingItems.forEach(i=>i.remove());
  const empty=$('conversationsEmpty'); if(empty) empty.style.display='none';

  try{
    /* Load existing conversations with latest message */
    ...
    const convMap={};
    convSnap.docs.forEach(doc=>{ const d=doc.data(); const otherUid=(d.participants||[]).find(u=>u!==currentUser.uid)||''; convMap[otherUid]={id:doc.id,...d}; });

    /* Load all registered users */
    const usersSnap=await DB.collection('users').orderBy('displayName').limit(50).get();
    ...
    /* Sort: users with conversations first, then others */
    const withConv=allUsers.filter(u=>convMap[u.id]).sort((a,b)=>{ const ta=(convMap[a.id].lastMessageAt&&convMap[a.id].lastMessageAt.toDate?convMap[a.id].lastMessageAt.toDate():new Date(0)); const tb=(convMap[b.id].lastMessageAt&&convMap[b.id].lastMessageAt.toDate?convMap[b.id].lastMessageAt.toDate():new Date(0)); return tb-ta; });
    const withoutConv=allUsers.filter(u=>!convMap[u.id]);
    const sorted=[...withConv,...withoutConv];
    ...
    sorted.forEach(u=>{
      ...
      const avHtml=u.photoURL?`<img src="${esc(u.photoURL)}" alt="" onerror="this.style.display='none'"/>`:name[0].toUpperCase();
      item.innerHTML=`
        <div class="conv-avatar" style="background:${randomColor(u.id)}">${avHtml}<div class="conv-online-dot" style="display:none"></div></div>
        <div class="conv-info">
          <p class="conv-name">${esc(name)}</p>
          <p class="conv-last-msg${unread?' unread':''}">${lastMsg!==null?esc(lastMsg):'<span style="color:var(--tm);font-style:italic">Tap to message</span>'}</p>
        </div>
        <div class="conv-meta">
          ${lastTime?'<span class="conv-time">'+fmtAgo(lastTime)+'</span>':''}
    ...
    chatUnsub=convRef.collection('messages').orderBy('createdAt','asc').limit(100)
      .onSnapshot(snap=>{
        ...
        snap.docs.forEach(doc=>{
          ...
          /* WhatsApp checkmarks */
          const readMark=isSent?`<span class="msg-read-tick" style="font-size:13px;color:${msg.read?'#53bdeb':'rgba(255,255,255,.6)'}">✓✓</span>`:'';
          let mHtml='';
          if(msg.mediaUrl){
            if(msg.mediaType==='image') mHtml=`<div class="chat-msg-media"><img src="${esc(msg.mediaUrl)}" loading="eager" onclick="openPhotoViewer(this.src,'',null)" style="cursor:pointer"/></div>`;
            else if(msg.mediaType==='video') mHtml=`<div class="chat-msg-media"><video src="${esc(msg.mediaUrl)}" controls playsinline muted style="border-radius:8px;max-width:100%"></video></div>`;
          }
          ...
          /* Long press / right click to delete own message */
          if(isSent){
            let pressTimer=null;
            el.addEventListener('touchstart',()=>{pressTimer=setTimeout(()=>confirmDeleteMsg(doc.id,convRef),600);});
            el.addEventListener('touchend',()=>clearTimeout(pressTimer));
            el.addEventListener('contextmenu',e=>{e.preventDefault();confirmDeleteMsg(doc.id,convRef);});
          }
          cm.appendChild(el);
          /* Mark as read */
        ...
        const upd={};upd['unreadCounts.'+currentUser.uid]=0;convRef.update(upd).catch(()=>{});
      },()=>{});
  }).catch(()=>{});
}

function confirmDeleteMsg(msgId,convRef){
  if(confirm('Delete this message?')){ convRef.collection('messages').doc(msgId).delete().catch(()=>{}); }
}
...
async function sendChatMessage(){
  ...
  try{
    ...
    if(file){
      ...
      if(mf) mf.value=''; setStyle('chatMediaPreview','display','none'); hideLoading();
    }
    ...
    DB.collection('notifications').add({toUid:activeChatUid,fromUid:currentUser.uid,fromName:myName,type:'message',read:false,createdAt:TS()}).catch(()=>{});
...
on('chatFileInput','change',e=>{ const file=e.target.files&&e.target.files[0];if(!file) return; if(file.size>2*1024*1024*1024){toast('Max 2GB for video','error');return;} const blob=URL.createObjectURL(file); const prev=$('chatMediaPreview'),pi=$('chatMediaPreviewImg'),pv=$('chatMediaPreviewVid'); if(prev) prev.style.display='block'; if(file.type.startsWith('image/')){if(pi){pi.src=blob;pi.style.display='block';}if(pv) pv.style.display='none';}else{if(pv){pv.src=blob;pv.style.display='block';}if(pi) pi.style.display='none';} });
...
on('newMessageBtn','click',()=>toast('Search for users above to start a chat','info'));
...
on('publishProductBtn','click',async()=>{
  ...
  const title=(($('productTitle')||{}).value||'').trim(),desc=(($('productDesc')||{}).value||'').trim();
  ...
  setStyle('adminProgressCard','display','block');setHTML('adminUploadLog','');setProgress(0,'Preparing...');uploadLog('Starting...','inf');
  const total=imgF.length+(type==='ebook'?1:0)+(type==='audio'?1:0)+1;let done=0;const imgUrls=[];
  try{
    for(let i=0;i<imgF.length;i++){setProgress(Math.round(done/total*100),'Image '+(i+1)+'/'+imgF.length);const url=await uploadFile(imgF[i],'products/images/',pct=>setProgress(Math.round((done+pct/100)/total*100),'Image '+(i+1)+': '+pct+'%'));imgUrls.push(url);done++;uploadLog('Image '+(i+1)+' uploaded','ok');}
    ...
    ['productTitle','productDesc','productMRP','productSalePrice'].forEach(id=>{const el=$(id);if(el) el.value='';});setHTML('adminImgPreviews','');
...
document.addEventListener('keydown',e=>{
  ...
  const pv=document.getElementById('photoViewerOverlay');if(pv&&pv.style.display==='flex'){pv.style.display='none';document.body.style.overflow='';return;}
  ...
  ['editProfileOverlay','settingsOverlay','notifOverlay','emailLoginOverlay'].forEach(id=>{ const ov=document.getElementById(id); if(ov&&(ov.classList.contains('upo-open')||ov.style.display==='flex')){ov.classList.remove('upo-open');ov.style.display='none';document.body.style.overflow='';} });
  const cH=document.getElementById('commentsHalfOverlay');if(cH&&cH.style.display==='flex'){cH.style.display='none';}
  const pm=document.getElementById('postMenuOverlay');if(pm) pm.remove();
});
...
console.log('%cSayatX v4.0 Ready!','color:#f97316;font-weight:900;font-size:16px');
