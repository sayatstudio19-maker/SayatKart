'use strict';
/* ═══════════════════════════════════════════════════
   SAYATX — app.js  v12.0   COMPLETE
   All features: Phone+Email login, Video aspect ratio,
   Buffering spinner, Drag-to-seek, Follow in posts/reels,
   Trading boost, Notification fix, Followers list,
   YouTube thumb picker, Sponsor link, Background upload
═══════════════════════════════════════════════════ */
...
const TRADE_KEYWORDS=['trading','trade','stock','forex','crypto','invest','market','share','nifty','sensex','bitcoin','eth','profit','earn','money','financial','finance'];
function isTrading(text){const t=(text||'').toLowerCase();return TRADE_KEYWORDS.some(k=>t.includes(k));}
...
.post-video-wrap.landscape video{width:100%;height:100%;object-fit:contain;display:block;background:#000;}
...
/* Trading badge */
.trading-badge{background:linear-gradient(90deg,#22c55e,#06b6d4);color:#fff;font-size:.64rem;font-weight:800;padding:2px 7px;border-radius:99px;margin-left:5px;}
/* Thumbnail section */
...
function updateLogos(){
  qsa('.header-logo,.drawer-logo,.auth-logo-icon').forEach(el=>{
    ...
    const img=document.createElement('img');img.src=LOGO_URL;img.alt='SayatX';img.setAttribute('data-sx','1');
  ...
  qsa('.header-brand-name,.drawer-brand-name,.brand-name-el').forEach(el=>{
    if(!el.querySelector('.brand-x')){const t=(el.textContent||'SayatX').trim(),prefix=t.endsWith('X')?t.slice(0,-1):t;el.innerHTML=`<span style="color:var(--txt)">${esc(prefix)}</span><span class="brand-x">X</span>`;}
  });
}
...
/* YouTube-style video frame picker */
function openVideoThumbPicker(videoFile,onSelected){
  ...
  ov.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:rgba(0,0,0,.9);flex-shrink:0;">
      <button id="vtpCancel" style="color:#fff;background:none;border:none;cursor:pointer;display:flex;align-items:center;gap:5px;font-size:.9rem"><span class="material-icons-round btn-anim">close</span>Cancel</button>
      <span style="color:#fff;font-weight:800">Pick Thumbnail Frame</span>
      <button id="vtpUse" style="color:#f97316;background:none;border:none;cursor:pointer;font-weight:800;font-size:.9rem">✔ Use</button>
    </div>
    <video id="vtpVid" src="${blobUrl}" playsinline muted style="flex:1;width:100%;object-fit:contain;background:#000;display:block;min-height:0;"></video>
    <div style="padding:12px 16px;background:rgba(0,0,0,.9);flex-shrink:0;display:flex;align-items:center;gap:10px;">
      <span id="vtpTime" style="color:#fff;font-size:.8rem;min-width:38px">0:00</span>
      <input type="range" id="vtpSeek" min="0" max="100" value="0" step="0.01" style="flex:1;accent-color:#f97316"/>
      <button id="vtpPlay" style="width:36px;height:36px;border-radius:50%;background:var(--pr);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center"><span class="material-icons-round" style="font-size:18px">play_arrow</span></button>
    </div>`;
  ...
  ov.querySelector('#vtpUse').addEventListener('click',()=>{
    vid.pause();
    const c=document.createElement('canvas');c.width=vid.videoWidth||640;c.height=vid.videoHeight||360;
    c.getContext('2d').drawImage(vid,0,0,c.width,c.height);
    c.toBlob(blob=>{URL.revokeObjectURL(blobUrl);ov.remove();if(blob&&onSelected)onSelected(blob);},'image/jpeg',0.88);
  });
}
  ...
  sc.innerHTML=`<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:16px;"><div style="background:var(--card);border:1px solid var(--bdr);border-radius:22px;padding:26px 20px;width:100%;max-width:400px;">
    ...
    <div id="authPanelPhone">
      ...
      <div id="otpWrap" style="display:none;margin-top:14px">
        <p style="font-size:.82rem;color:var(--tm);text-align:center;margin-bottom:10px">Enter 6-digit OTP</p>
    ...
    <div id="authPanelEmail" style="display:none">
      ...
      <input id="emPass" type="password" placeholder="Password (min 6 chars)" style="width:100%;padding:12px;border:1.5px solid var(--bdr2);border-radius:var(--rf);background:var(--card2);color:var(--txt);font-size:.9rem;outline:none;margin-bottom:10px;box-sizing:border-box"/>
...
function sendOtp(){
  ...
  try{
    rcaptchaVerifier=new firebase.auth.RecaptchaVerifier('recaptcha-container',{size:'normal',callback:()=>{
...
function verifyOtp(){
  ...
  rcaptchaResult.confirm(otp).then(()=>{toast('Phone verified!','success');switchSection('home');showProfileSetupIfNeeded();}).catch(()=>toast('Invalid OTP','error')).finally(hideLoading);
}
function showProfileSetupIfNeeded(){
  ...
  setTimeout(()=>{
    ...
    ov.innerHTML=`<div style="background:var(--card);border-radius:20px;padding:24px;width:100%;max-width:380px"><h3 style="font-weight:900;font-size:1.1rem;color:var(--txt);margin-bottom:6px">Set up your profile</h3><p style="font-size:.82rem;color:var(--tm);margin-bottom:18px">Optional — you can skip this</p><input id="setupName" type="text" placeholder="Your display name" maxlength="40" style="width:100%;padding:11px 13px;border:1.5px solid var(--bdr2);border-radius:var(--rf);background:var(--card2);color:var(--txt);font-size:.9rem;outline:none;box-sizing:border-box;margin-bottom:10px"/><div style="display:flex;gap:8px"><button id="setupSkip" style="flex:1;padding:12px;background:var(--card2);border:1.5px solid var(--bdr2);border-radius:var(--rf);font-size:.88rem;font-weight:700;color:var(--tm);cursor:pointer">Skip</button><button id="setupSave" style="flex:2;padding:12px;background:var(--pr);color:#fff;border-radius:var(--rf);font-size:.88rem;font-weight:800;border:none;cursor:pointer">Save & Continue</button></div></div>`;
    document.body.appendChild(ov);
    ov.querySelector('#setupSkip').onclick=()=>ov.remove();
    ov.querySelector('#setupSave').onclick=async()=>{const n=(ov.querySelector('#setupName').value||'').trim();if(!n||n.length<2){toast('Enter at least 2 characters','error');return;}showLoading('Saving...');try{await currentUser.updateProfile({displayName:n});await DB.collection('users').doc(currentUser.uid).update({displayName:n,updatedAt:TS()});localStorage.setItem('sx_name',n);refreshProfileUI();updateDrawerStrip(currentUser);ov.remove();toast('Profile set!','success');}catch(e){toast('Failed: '+e.message,'error');}finally{hideLoading();}};
  },600);
}
...
/* ════════════════════════════════════
   NOTIFICATIONS — with fallback for missing index
════════════════════════════════════ */
...
async function loadNotifications(){
  ...
  try{
    ...
    try{const snap=await DB.collection('notifications').where('toUid','==',currentUser.uid).limit(40).get();docs=snap.docs.sort((a,b)=>{const ta=a.data().createdAt?.toDate?a.data().createdAt.toDate():new Date(0);const tb=b.data().createdAt?.toDate?b.data().createdAt.toDate():new Date(0);return tb-ta;});}
    catch(e2){body.innerHTML='<p style="padding:20px;color:var(--tm)">Failed to load. Check Firestore index.</p>';return;}
  }
  body.innerHTML='';
  ...
  docs.forEach(doc=>{
    ...
    const rb=info.querySelector('.notif-reply-btn');if(rb){rb.onclick=e=>{e.stopPropagation();const no=document.getElementById('notifOverlay');if(no)no.classList.remove('upo-open');document.body.style.overflow='';switchSection('messages');setTimeout(()=>openChatWith(n.fromUid,n.fromName,n.fromPhoto),300);};}
...
function loadFeed(filter){
  ...
  feedUnsub=q.onSnapshot(snap=>{
    ...
    /* Boost trading videos to top */
    if(filter==='all'||filter==='videos'||filter==='trending'){
      const tradingPosts=posts.filter(p=>isTrading(p.text)&&p.mediaType==='video');
      const otherPosts=posts.filter(p=>!(isTrading(p.text)&&p.mediaType==='video'));
      posts=[...tradingPosts,...otherPosts];
    }
    let items=[...posts];
...
async function quickFollow(targetUid,btn){
  ...
  try{
    ...
    if(isNowFollowing){
      ...
      batch.update(DB.collection('users').doc(targetUid),{followers:INC(-1)});
      batch.update(DB.collection('users').doc(currentUser.uid),{following:INC(-1)});
      followingCache[targetUid]=false;
    }else{
      batch.set(DB.collection('follows').doc(docId),{followerUid:currentUser.uid,followingUid:targetUid,createdAt:TS()});
      batch.update(DB.collection('users').doc(targetUid),{followers:INC(1)});
      batch.update(DB.collection('users').doc(currentUser.uid),{following:INC(1)});
      followingCache[targetUid]=true;
      DB.collection('notifications').add({toUid:targetUid,fromUid:currentUser.uid,fromName:currentUser.displayName||'User',fromPhoto:currentUser.photoURL||'',type:'follow',read:false,createdAt:TS()}).catch(()=>{});
    }
    await batch.commit();
    ...
    toast(!isNowFollowing?'Following!':'Unfollowed','success');
...
function buildUserPost(post){
  ...
  const fBtn=el.querySelector('.quick-follow-btn');
  if(fBtn){fBtn.addEventListener('click',e=>{e.stopPropagation();quickFollow(post.userId,fBtn);});}
  ...
  /* Video: aspect ratio + buffer spinner + click to open reels */
...
/* ════════════════════════════════════
   REELS — drag to seek, buffer spinner, follow button
════════════════════════════════════ */
  ...
  posts.forEach((post,i)=>{
    ...
    item.innerHTML=`
      ...
      <div class="reel-side-btns">
        ...
        ${!isOwn?`<div class="reel-side-btn reel-follow-btn" data-uid="${esc(post.userId||'')}"><span class="material-icons-round">${isFollowingUser?'person_remove':'person_add'}</span><span class="reel-count">${isFollowingUser?'Following':'Follow'}</span></div>`:''}
    ...
    vid.addEventListener('touchmove',e=>{
      ...
      if(Math.abs(dx)>15){isDragging=true;const seek=Math.max(0,Math.min(vid.duration||0,seekStartTime+dx*0.15));vid.currentTime=seek;if(seekHint){seekHint.textContent=(dx>0?'+':'')+Math.round(dx*0.15)+'s';seekHint.classList.add('show');}}
    },{passive:true});
    vid.addEventListener('touchend',()=>{isDragging=false;if(seekHint)setTimeout(()=>seekHint.classList.remove('show'),600);});

    /* Tap: pause/play. Double tap: like. No click if dragging. */
    let tt=null,tc=0;
    item.addEventListener('click',e=>{
      if(isDragging)return;
      if(e.target.closest('.reel-side-btns')||e.target.closest('.reel-info-user')||e.target.closest('.reel-seek'))return;
      tc++;clearTimeout(tt);tt=setTimeout(()=>{if(tc===1){if(vid.paused)vid.play().catch(()=>{});else vid.pause();}else if(tc>=2){handleReelLike(post.id,item.querySelector('.reel-like-btn'));showHeartEffect(item);}tc=0;},280);
    });
    ...
    /* Follow button in reel */
    const reelFollowBtn=item.querySelector('.reel-follow-btn');
    if(reelFollowBtn){
      reelFollowBtn.addEventListener('click',e=>{
        e.stopPropagation();
        const uid=reelFollowBtn.dataset.uid;if(!uid)return;
        const isNowFollowing=!!followingCache[uid];
        const fakeBtn=document.createElement('button');fakeBtn.classList.toggle('following',isNowFollowing);
        quickFollow(uid,fakeBtn).then?.(()=>{}).catch?.(()=>{});
        /* Update UI optimistically */
        followingCache[uid]=!isNowFollowing;localStorage.setItem('sx_following',JSON.stringify(followingCache));
        const icon=reelFollowBtn.querySelector('.material-icons-round'),label=reelFollowBtn.querySelector('.reel-count');
        if(icon)icon.textContent=!isNowFollowing?'person_remove':'person_add';if(label)label.textContent=!isNowFollowing?'Following':'Follow';
        /* Actually call quickFollow */
        quickFollow(uid,{classList:{contains:()=>isNowFollowing,toggle:()=>{},add:()=>{},remove:()=>{}},textContent:'',disabled:false});
      });
    }
...
/* ════════════════════════════════════
   CREATE POST — thumbnail + YouTube frame picker
════════════════════════════════════ */
...
function setCreateTab(type){
  ...
  if(sheet&&!sheet.querySelector('.create-thumb-section')){
    ...
    ts.querySelector('#createThumbBtn').addEventListener('click',()=>{
      if(createPostType==='video'&&createMediaFile){
        openVideoThumbPicker(createMediaFile,blob=>{
          ...
          const w=ts.querySelector('#createThumbPreviewWrap'),img=ts.querySelector('#createThumbPreviewImg');
          if(w){w.style.display='inline-block';}if(img)img.src=createThumbBlob;
          toast('Thumbnail selected!','success');
        });
      }else{ts.querySelector('#createThumbInput').click();}
    });
...
on('createSubmitBtn','click',async()=>{
  ...
  let docRef=null;
  try{docRef=await DB.collection('posts').add({userId:currentUser.uid,userName:name,userPhoto:currentUser.photoURL||'',text:text||'',mediaType:file?createPostType:'text',mediaUrl:null,thumbnailUrl:null,likes:0,commentCount:0,views:0,status:'uploading',uploadPct:0,createdAt:TS()});await DB.collection('users').doc(currentUser.uid).update({postCount:INC(1)}).catch(()=>{});}catch(e){toast('Post failed: '+e.message,'error',5000);if(btn)btn.disabled=false;return;}
  ...
  try{
    ...
    const mediaUrl=await uploadFile(file,folder,async pct=>{if(pct%5===0&&docRef){try{await docRef.update({uploadPct:pct});}catch(e){}}if(liveToast&&liveToast.isConnected)liveToast.textContent='Uploading... '+pct+'%';});
...
/* ════════════════════════════════════
   USER PROFILE — real photo, audio continues, FB posts, followers
════════════════════════════════════ */
...
function openUserProfile(uid){
  ...
  if(newF&&followBtn){
    ...
    newF.addEventListener('click',async()=>{
      ...
      newF.disabled=true;
      try{
        const batch=DB.batch(),docId=currentUser.uid+'_'+uid;
        if(isFollowing){batch.delete(DB.collection('follows').doc(docId));batch.update(DB.collection('users').doc(uid),{followers:INC(-1)});batch.update(DB.collection('users').doc(currentUser.uid),{following:INC(-1)});await batch.commit();isFollowing=false;followingCache[uid]=false;newF.classList.remove('btn-following');newF.style.background='var(--txt)';newF.style.color='var(--bg)';newF.innerHTML='<span class="material-icons-round">person_add</span> Follow';toast('Unfollowed','info');}
        else{batch.set(DB.collection('follows').doc(docId),{followerUid:currentUser.uid,followingUid:uid,createdAt:TS()});batch.update(DB.collection('users').doc(uid),{followers:INC(1)});batch.update(DB.collection('users').doc(currentUser.uid),{following:INC(1)});await batch.commit();isFollowing=true;followingCache[uid]=true;newF.classList.add('btn-following');newF.style.background='transparent';newF.style.color='var(--txt)';newF.style.border='1.5px solid var(--bdr2)';newF.innerHTML='<span class="material-icons-round">person_remove</span> Following';toast('Following!','success');DB.collection('notifications').add({toUid:uid,fromUid:currentUser.uid,fromName:currentUser.displayName||'User',fromPhoto:currentUser.photoURL||'',type:'follow',read:false,createdAt:TS()}).catch(()=>{});}
        localStorage.setItem('sx_following',JSON.stringify(followingCache));
  ...
  DB.collection('users').doc(uid).get().then(snap=>{
    ...
    /* Load posts — audio keeps playing (don't stop it) */
...
/* ════════════════════════════════════
   MESSAGES — last sender profile at top, all users shown
════════════════════════════════════ */
...
console.log('%cSayatX v12.0 Ready! All features loaded.','color:#f97316;font-weight:900;font-size:16px');
