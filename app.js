...
/* ═══════════════════════════════════════════════════
   SAYATX — app.js  v7.0
   Guest-only login · Bigger autoplay sponsors with link
   Facebook-style post lists · Messages fixed · Notifications
═══════════════════════════════════════════════════ */
...
/* ── Logo replace everywhere (SS → real logo) ── */
function updateLogos(){
  qsa('img[data-logo]').forEach(img=>img.src=LOGO_URL);
  /* Replace any element that literally shows "SS" text-only logo box */
  qsa('.header-logo, .drawer-logo, .auth-logo-icon').forEach(el=>{
    if(el.querySelector('img[data-injected-logo]')) return;
    el.innerHTML='';
    const img=document.createElement('img');
    img.src=LOGO_URL; img.alt='SayatX'; img.setAttribute('data-injected-logo','1');
    img.style.cssText='width:100%;height:100%;object-fit:contain;border-radius:inherit;';
    el.appendChild(img);
  });
  qsa('.brand-name-el,.header-brand-name,.drawer-brand-name').forEach(el=>{
...
setTimeout(updateLogos,80);
setTimeout(updateLogos,400);
...
function uploadFile(file,folder,onProgress){
  return new Promise((resolve,reject)=>{
    ...
    /* expose task globally so we don't cancel on section switch */
    window._activeUploadTasks=window._activeUploadTasks||[];
    window._activeUploadTasks.push(task);
  });
}
/* Warn before closing tab if upload in progress */
window.addEventListener('beforeunload',e=>{
  const tasks=window._activeUploadTasks||[];
  const active=tasks.some(t=>t.snapshot&&t.snapshot.state==='running');
...
function ensureUserDoc(user){ if(!user) return; const ref=DB.collection('users').doc(user.uid); ref.get().then(snap=>{ const name=user.displayName||'Guest'+user.uid.slice(0,5); const data={uid:user.uid,displayName:name,photoURL:user.photoURL||'',isAnonymous:true,updatedAt:TS()}; if(!snap.exists) ref.set({...data,followers:0,following:0,postCount:0,createdAt:TS()}); else ref.update(data); }).catch(()=>{}); }

function updateDrawerStrip(user){ if(!user) return; const name=user.displayName||'Guest'; setText('drawerUserName',name);setText('drawerUserHandle','@'+name.toLowerCase().replace(/[^a-z0-9]/g,'_')); const img=$('drawerUserAvatarImg'),ltr=$('drawerUserAvatarLetter'); if(user.photoURL){if(img){img.src=user.photoURL;img.style.display='block';}if(ltr) ltr.style.display='none';}else{if(ltr){ltr.textContent=name[0].toUpperCase();ltr.style.display='flex';}if(img) img.style.display='none';} DB.collection('users').doc(user.uid).get().then(snap=>{if(!snap.exists) return;const d=snap.data();setText('drawerPostCount',fmtCount(d.postCount||0));setText('drawerFollowerCount',fmtCount(d.followers||0));}).catch(()=>{}); }

function refreshProfileUI(){ if(!currentUser) return; const name=currentUser.displayName||'Guest'; setText('profileDisplayName',name);setText('profileHandle','@'+name.toLowerCase().replace(/[^a-z0-9]/g,'_')); const img=$('profileAvatarImg'),ltr=$('profileAvatarLetter'); if(currentUser.photoURL){if(img){img.src=currentUser.photoURL;img.style.display='block';}if(ltr) ltr.style.display='none';}else{if(ltr){ltr.textContent=name[0].toUpperCase();ltr.style.display='flex';}if(img) img.style.display='none';} DB.collection('users').doc(currentUser.uid).get().then(snap=>{if(!snap.exists) return;const d=snap.data();setText('profilePostCount',fmtCount(d.postCount||0));setText('profileFollowerCount',fmtCount(d.followers||0));setText('profileFollowingCount',fmtCount(d.following||0));}).catch(()=>{}); }

/* ── Build auth screen — GUEST ONLY ── */
function buildAuthScreen(){
  const authScreen=$('authScreen'); if(!authScreen) return;
  authScreen.innerHTML=`
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;">
  <div style="background:var(--card);border:1px solid var(--bdr);border-radius:24px;padding:32px 24px;width:100%;max-width:380px;text-align:center;">
    <div style="width:76px;height:76px;border-radius:20px;overflow:hidden;margin:0 auto 16px;box-shadow:0 4px 20px rgba(249,115,22,.3)"><img src="${LOGO_URL}" style="width:100%;height:100%;object-fit:contain" alt="SayatX"/></div>
    <h1 style="font-family:'Baloo 2',cursive;font-size:2rem;margin-bottom:6px"><span style="color:var(--txt)">Sayat</span><span class="brand-x">X</span></h1>
    <p style="font-size:.88rem;color:var(--tm);margin-bottom:28px">Share your world</p>
    <button id="anonLoginBtn" style="width:100%;padding:15px;background:var(--pr);color:#fff;border-radius:var(--rf);font-size:.95rem;font-weight:800;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:9px;box-shadow:0 4px 14px rgba(249,115,22,.3)">
      <span class="material-icons-round" style="font-size:20px">login</span>Enter SayatX
    </button>
    <p style="margin-top:16px;font-size:.78rem;color:var(--tm)">Tap to continue — no signup needed</p>
  </div>
  </div>`;
  on('anonLoginBtn','click',async()=>{
    showLoading('Entering SayatX...');
    try{ await Auth.signInAnonymously(); toast('Welcome to SayatX!','success'); switchSection('home'); }
    catch(e){ toast('Failed: '+e.message,'error'); }
    finally{ hideLoading(); }
  });
}

async function handleLogout(){ showLoading('Logging out...'); if(chatUnsub){chatUnsub();chatUnsub=null;} if(ordersUnsub){ordersUnsub();ordersUnsub=null;} if(notifUnsub){notifUnsub();notifUnsub=null;} await Auth.signOut().catch(()=>{}); toast('Logged out','info');switchSection('home');hideLoading(); buildAuthScreen(); }
...
/* ══════════════════════════════════════
   NOTIFICATIONS — red bell, message reply
══════════════════════════════════════ */
...
async function loadNotifications(){
  ...
  try{
    const snap=await DB.collection('notifications').where('toUid','==',currentUser.uid).orderBy('createdAt','desc').limit(40).get();
    body.innerHTML='';
    ...
    snap.docs.forEach(doc=>{
      ...
      el.style.cssText='display:flex;align-items:flex-start;gap:12px;padding:13px 16px;border-bottom:1px solid var(--bdr);';
      const avWrap=document.createElement('div');
      avWrap.style.cssText='width:42px;height:42px;border-radius:50%;background:'+randomColor(n.fromUid)+';display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:1rem;flex-shrink:0;overflow:hidden;cursor:pointer;';
      if(n.fromPhoto&&n.fromPhoto.startsWith('http')){ avWrap.innerHTML=`<img src="${esc(n.fromPhoto)}" style="width:100%;height:100%;object-fit:cover" onerror="this.parentNode.textContent='${(n.fromName||'U')[0].toUpperCase()}'"/>`; }
      else avWrap.textContent=(n.fromName||'U')[0].toUpperCase();
      avWrap.addEventListener('click',()=>{ const notifOv=document.getElementById('notifOverlay');if(notifOv) notifOv.classList.remove('upo-open');document.body.style.overflow='';openUserProfile(n.fromUid); });
      const typeText=n.type==='follow'?'started following you':n.type==='like'?'liked your post':n.type==='comment'?'commented on your post':n.type==='message'?'sent you a message':'interacted with you';
      const info=document.createElement('div'); info.style.flex='1';
      const msgPreview=n.type==='message'&&n.preview?`<p style="font-size:.8rem;color:var(--txt);margin-top:3px;background:var(--card2);padding:6px 10px;border-radius:10px;display:inline-block">"${esc(n.preview)}"</p>`:'';
      const replyBtn=n.type==='message'?`<button style="margin-top:6px;padding:6px 14px;background:var(--pr);color:#fff;border:none;border-radius:var(--rf);font-size:.78rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;width:fit-content"><span class="material-icons-round" style="font-size:14px">reply</span>Reply</button>`:'';
      info.innerHTML=`<p style="font-weight:700;font-size:.88rem;color:var(--txt)">${esc(n.fromName||'User')} <span style="font-weight:500;color:var(--tm)">${typeText}</span></p><p style="font-size:.74rem;color:var(--tm);margin-bottom:2px">${fmtAgo(n.createdAt)}</p>${msgPreview}<div>${replyBtn}</div>`;
      el.appendChild(avWrap); el.appendChild(info);
      const go=()=>{ const notifOv=document.getElementById('notifOverlay');if(notifOv) notifOv.classList.remove('upo-open');document.body.style.overflow=''; if(n.type==='message'){switchSection('messages');setTimeout(()=>openChatWith(n.fromUid,n.fromName,n.fromPhoto),300);}else if(n.type==='follow'){openUserProfile(n.fromUid);} };
      const rb=info.querySelector('button'); if(rb) rb.addEventListener('click',e=>{e.stopPropagation();go();});
      info.style.cursor='pointer';
      info.addEventListener('click',e=>{ if(e.target.closest('button')) return; go(); });
      body.appendChild(el);
    });
...
/* ══════════════════════════════════════
   FEATURED — bigger, link-based, autoplay+sound
══════════════════════════════════════ */
  ...
  DB.collection('featured').orderBy('createdAt','desc').limit(10).onSnapshot(snap=>{
    ...
    snap.docs.forEach(doc=>{
      ...
      if(isVideo&&d.mediaUrl){
        card.innerHTML=`<video src="${esc(d.mediaUrl)}" style="width:100%;height:100%;object-fit:cover;display:block;" autoplay loop playsinline muted></video><div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.8) 0%,transparent 50%);display:flex;flex-direction:column;justify-content:flex-end;padding:12px"><p style="color:#fff;font-size:.92rem;font-weight:800;margin-bottom:4px">${esc(d.title||'Sponsored')}</p><span style="font-size:.7rem;color:rgba(255,255,255,.7);display:flex;align-items:center;gap:3px"><span class="material-icons-round" style="font-size:12px">star</span>Sponsored</span></div><button class="featured-mute-btn" style="position:absolute;top:10px;right:10px;width:30px;height:30px;border-radius:50%;background:rgba(0,0,0,.5);border:none;display:flex;align-items:center;justify-content:center;color:#fff;cursor:pointer;z-index:2"><span class="material-icons-round" style="font-size:16px">volume_off</span></button>`;
        const vid=card.querySelector('video');
        const muteBtn=card.querySelector('.featured-mute-btn');
        muteBtn.addEventListener('click',e=>{ e.stopPropagation(); vid.muted=!vid.muted; muteBtn.querySelector('.material-icons-round').textContent=vid.muted?'volume_off':'volume_up'; });
      }else{
        const thumb=d.thumbnailUrl||'';
        const icon=isAudio?'headphones':'image';
        card.innerHTML=(thumb?`<img src="${esc(thumb)}" style="width:100%;height:100%;object-fit:cover;display:block;" onerror="this.style.display='none'"/>`:'')+`<div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.75) 0%,transparent 50%);display:flex;flex-direction:column;justify-content:flex-end;padding:12px"><p style="color:#fff;font-size:.92rem;font-weight:800;margin-bottom:4px">${esc(d.title||'Sponsored')}</p><span style="font-size:.7rem;color:rgba(255,255,255,.7);display:flex;align-items:center;gap:3px"><span class="material-icons-round" style="font-size:12px">star</span>Sponsored</span></div>`;
      }
      /* Click → open link if exists, else play media */
      card.addEventListener('click',e=>{
        if(e.target.closest('.featured-mute-btn')) return;
        if(d.link){ window.open(d.link,'_blank','noopener'); }
        else if(isVideo&&d.mediaUrl){ openReels([{id:d.id,mediaUrl:d.mediaUrl,mediaType:'video',text:d.title||'',userName:'SayatX',userPhoto:LOGO_URL,thumbnailUrl:d.thumbnailUrl||'',likes:0,commentCount:0,views:0,createdAt:d.createdAt}],0); }
...
/* Build user post — video autoplay click=sound everywhere */
function buildUserPost(post){
  ...
  let media='';
  if(post.mediaType==='video'&&post.mediaUrl){ media=`<div class="post-video-wrap" data-post-id="${post.id}"><video class="post-video-thumb" src="${esc(post.mediaUrl)}" preload="none" playsinline muted loop poster="${esc(post.thumbnailUrl||'')}"></video><div class="post-video-play-overlay"><div class="post-play-btn"><span class="material-icons-round">play_arrow</span></div></div><div class="post-vid-badges"><span class="post-vid-badge"><span class="material-icons-round">videocam</span>Video</span></div><button class="post-video-mute-btn" style="position:absolute;bottom:10px;right:10px;width:30px;height:30px;border-radius:50%;background:rgba(0,0,0,.5);border:none;display:flex;align-items:center;justify-content:center;color:#fff;cursor:pointer;z-index:3"><span class="material-icons-round" style="font-size:16px">volume_off</span></button></div>`; }
  ...
  /* Video: click = unmute+sound (toggles play if needed), double tap = like */
  const vw=el.querySelector('.post-video-wrap');
  if(vw){
    const vid=vw.querySelector('video');
    const muteBtn=vw.querySelector('.post-video-mute-btn');
    let tt=null,tc=0;
    vw.addEventListener('click',e=>{
      if(e.target.closest('.post-video-mute-btn')) return;
      tc++;clearTimeout(tt);
      tt=setTimeout(()=>{
        if(tc===1){
          /* Click anywhere in feed → open reels with sound */
    ...
    if(muteBtn) muteBtn.addEventListener('click',e=>{ e.stopPropagation(); vid.muted=!vid.muted; muteBtn.querySelector('.material-icons-round').textContent=vid.muted?'volume_off':'volume_up'; });
  }
...
/* Comments half-screen — newest first */
...
/* ══════════════════════════════════════
   CREATE POST — thumbnail, % progress, survives backgrounding
══════════════════════════════════════ */
...
/* ══════════════════════════════════════
   FACEBOOK-STYLE POST LIST BUILDER
   Used both for own profile (Your Posts) and other user's profile
══════════════════════════════════════ */
function buildFbStylePostItem(p, userInfo, onDeleteCallback){
  /* userInfo = {name, photoURL, uid} */
  const el=document.createElement('div');
  el.className='fb-post-list-item';
  el.style.cssText='background:var(--card);padding:12px 0;margin-bottom:10px;';
  const name=userInfo.name||'User';
  const avHtml=userInfo.photoURL&&userInfo.photoURL.startsWith('http')?`<img src="${esc(userInfo.photoURL)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.parentNode.textContent='${name[0].toUpperCase()}'"/>`:name[0].toUpperCase();

  if(p.status==='uploading'){
    el.innerHTML=`
      <div style="display:flex;align-items:center;gap:10px;padding:0 14px 10px;">
        <div style="width:38px;height:38px;border-radius:50%;background:${randomColor(userInfo.uid)};display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:.9rem;flex-shrink:0;overflow:hidden">${avHtml}</div>
        <div><p style="font-weight:800;font-size:.88rem;color:var(--txt)">${esc(name)}</p><p style="font-size:.74rem;color:var(--tm)">${fmtAgo(p.createdAt)}</p></div>
      </div>
      <div style="width:100%;aspect-ratio:16/10;background:var(--card2);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;">
        <div class="spinner"></div>
        <span style="font-size:.78rem;color:var(--tm);font-weight:700">Processing... ${p.uploadPct||0}%</span>
      </div>`;
    return el;
  }
  if(p.status==='failed'){
    el.innerHTML=`<div style="display:flex;align-items:center;gap:10px;padding:0 14px 10px;"><div style="width:38px;height:38px;border-radius:50%;background:${randomColor(userInfo.uid)};display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:.9rem;flex-shrink:0;overflow:hidden">${avHtml}</div><div><p style="font-weight:800;font-size:.88rem;color:var(--txt)">${esc(name)}</p><p style="font-size:.74rem;color:var(--er)">Upload failed</p></div></div>`;
    return el;
  }
  ...
  let mediaHtml='';
  if(isVid&&thumb){
    mediaHtml=`<div class="fb-vid-wrap" style="position:relative;width:100%;aspect-ratio:16/10;background:#000;cursor:pointer;overflow:hidden;">
      <img src="${esc(thumb)}" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none'"/>
      <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.25)">
        <div style="width:52px;height:52px;border-radius:50%;background:rgba(255,255,255,.92);display:flex;align-items:center;justify-content:center;"><span class="material-icons-round" style="font-size:28px;color:#000">play_arrow</span></div>
      </div>
    </div>`;
  }else if(isPhoto&&thumb){
    mediaHtml=`<img src="${esc(thumb)}" style="width:100%;max-height:420px;object-fit:cover;cursor:pointer;display:block;" loading="lazy"/>`;
  }else if(isAudio){
    mediaHtml=`<div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--card2);margin:0 14px;border-radius:12px;cursor:pointer;">${p.thumbnailUrl?`<img src="${esc(p.thumbnailUrl)}" style="width:48px;height:48px;border-radius:10px;object-fit:cover"/>`:`<div style="width:48px;height:48px;border-radius:10px;background:var(--pr);display:flex;align-items:center;justify-content:center;color:#fff"><span class="material-icons-round">headphones</span></div>`}<p style="font-weight:700;font-size:.86rem;color:var(--txt)">${esc(p.text||'Audio')}</p></div>`;
  }

  el.innerHTML=`
    <div style="display:flex;align-items:center;gap:10px;padding:0 14px 10px;">
      <div style="width:38px;height:38px;border-radius:50%;background:${randomColor(userInfo.uid)};display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:.9rem;flex-shrink:0;overflow:hidden;cursor:pointer" class="fb-item-avatar">${avHtml}</div>
      <div style="flex:1"><p style="font-weight:800;font-size:.88rem;color:var(--txt)">${esc(name)}</p><p style="font-size:.74rem;color:var(--tm)">${fmtAgo(p.createdAt)}</p></div>
      <button class="fb-item-3dot" style="width:30px;height:30px;border-radius:50%;background:none;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--tm)"><span class="material-icons-round" style="font-size:18px">more_vert</span></button>
    </div>
    ${p.text&&!isAudio?`<p style="padding:0 14px 10px;font-size:.86rem;color:var(--txt);white-space:pre-wrap;">${esc(p.text)}</p>`:''}
    ${mediaHtml}
    <div style="display:flex;align-items:center;gap:16px;padding:10px 14px 0;">
      <span style="font-size:.78rem;color:var(--tm);display:flex;align-items:center;gap:4px"><span class="material-icons-round" style="font-size:15px">favorite</span>${fmtCount(p.likes||0)}</span>
      <span style="font-size:.78rem;color:var(--tm);display:flex;align-items:center;gap:4px"><span class="material-icons-round" style="font-size:15px">chat_bubble_outline</span>${fmtCount(p.commentCount||0)}</span>
      <span style="font-size:.78rem;color:var(--tm);display:flex;align-items:center;gap:4px"><span class="material-icons-round" style="font-size:15px">visibility</span>${fmtCount(p.views||0)}</span>
    </div>`;

  /* Avatar click → profile */
  el.querySelector('.fb-item-avatar').addEventListener('click',e=>{ e.stopPropagation(); if(userInfo.uid) openUserProfile(userInfo.uid); });
  /* Media click → open viewer */
  const mediaEl=el.querySelector('.fb-vid-wrap') || (isPhoto?el.querySelector('img:not(.fb-item-avatar img)'):null) || (isAudio?el.querySelector('div[style*="cursor:pointer"]'):null);
  if(isVid){ const vw=el.querySelector('.fb-vid-wrap'); if(vw) vw.addEventListener('click',()=>{ openReels([p],0); }); }
  else if(isPhoto){ const img=el.querySelectorAll('img')[el.querySelectorAll('img').length-1]; if(img&&isPhoto) img.addEventListener('click',()=>openPhotoViewer(p.mediaUrl,p.text||'',p)); }
  else if(isAudio){ const ac=el.children[el.querySelector('.fb-item-3dot')?2:1]; /* fallback select by style */ const audioDiv=Array.from(el.children).find(c=>c.style&&c.style.cursor==='pointer'&&c.style.display==='flex'&&c.innerHTML.includes('headphones')); }
  /* simpler: query specifically */
  const audioCardEl=el.querySelector('div[style*="background:var(--card2)"]');
  if(isAudio&&audioCardEl){ audioCardEl.addEventListener('click',()=>openAudioOverlay(p.mediaUrl,p.text||'Audio',p.thumbnailUrl||'')); }

  /* 3-dot menu */
  el.querySelector('.fb-item-3dot').addEventListener('click',e=>{
    e.stopPropagation();
    const isOwn=currentUser&&userInfo.uid===currentUser.uid;
    const m=document.createElement('div');m.style.cssText='position:fixed;inset:0;z-index:660;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,.5);';
    const sh=document.createElement('div');sh.style.cssText='background:var(--card);border-radius:20px 20px 0 0;width:100%;max-width:520px;padding:16px 0 calc(env(safe-area-inset-bottom,0px)+16px);';
    const btns=[{icon:'share',label:'Share',fn:()=>handleShare(p.id,p.text||'SayatX')}];
    if(isOwn) btns.push({icon:'delete',label:'Delete Post',fn:()=>{ m.remove(); if(confirm('Delete this post?')){ deletePost(p.id).then(()=>{ if(onDeleteCallback) onDeleteCallback(); }); } },danger:true});
    btns.forEach(b=>{const btn2=document.createElement('button');btn2.style.cssText='display:flex;align-items:center;gap:14px;width:100%;padding:14px 20px;background:none;border:none;cursor:pointer;font-size:.9rem;font-weight:700;color:'+(b.danger?'var(--er)':'var(--txt)')+';';btn2.innerHTML='<span class="material-icons-round" style="color:'+(b.danger?'var(--er)':'var(--pr)')+'">'+b.icon+'</span>'+esc(b.label);btn2.addEventListener('click',()=>{m.remove();b.fn();});sh.appendChild(btn2);});
    const ca=document.createElement('button');ca.style.cssText='display:flex;align-items:center;justify-content:center;width:calc(100% - 32px);margin:8px 16px 0;padding:13px;background:var(--card2);border:none;border-radius:var(--rf);cursor:pointer;font-size:.9rem;font-weight:700;color:var(--txt);';ca.textContent='Cancel';ca.addEventListener('click',()=>m.remove());sh.appendChild(ca);
    m.appendChild(sh);m.addEventListener('click',ev=>{if(ev.target===m) m.remove();});document.body.appendChild(m);
  });
...
/* ══════════════════════════════════════
   USER PROFILE — Facebook-style post LIST + fix avatar + follow counts
══════════════════════════════════════ */
function openUserProfile(uid){
  ...
  const upoAv=$('upoAvatar'); if(upoAv) upoAv.innerHTML='<div class="spinner"></div>';
  setHTML('upoPostsGrid','<div class="pdf-loading" style="grid-column:1/-1"><div class="spinner"></div></div>');
  /* Switch grid to list layout */
  const grid=$('upoPostsGrid'); if(grid){ grid.style.display='block'; grid.style.gridTemplateColumns=''; grid.style.gap='0'; grid.style.padding='10px 0'; }
  ...
  if(currentUser&&!isOwn){
    DB.collection('follows').doc(currentUser.uid+'_'+uid).get().then(snap=>{
      ...
      if(followBtn){followBtn.classList.toggle('btn-following',isFollowing);followBtn.style.background=isFollowing?'transparent':'var(--txt)';followBtn.style.color=isFollowing?'var(--txt)':'var(--bg)';followBtn.innerHTML=isFollowing?'<span class="material-icons-round">person_remove</span> Following':'<span class="material-icons-round">person_add</span> Follow';}
    }).catch(()=>{});
  }
    ...
    newF.addEventListener('click',async()=>{
      ...
      try{
        if(isFollowing){
          ...
          isFollowing=false;newF.classList.remove('btn-following');newF.style.background='var(--txt)';newF.style.color='var(--bg)';newF.innerHTML='<span class="material-icons-round">person_add</span> Follow';
          ...
          isFollowing=true;newF.classList.add('btn-following');newF.style.background='transparent';newF.style.color='var(--txt)';newF.style.border='1.5px solid var(--bdr2)';newF.innerHTML='<span class="material-icons-round">person_remove</span> Following';
        ...
        /* Refresh displayed counts from server */
        DB.collection('users').doc(uid).get().then(s=>{if(s.exists) setText('upoFollowerCount',fmtCount(s.data().followers||0));}).catch(()=>{});
        if(currentUser) DB.collection('users').doc(currentUser.uid).get().then(s=>{if(s.exists) updateDrawerStrip(currentUser);}).catch(()=>{});
  ...
  DB.collection('users').doc(uid).get().then(snap=>{
    ...
    /* Load posts as Facebook-style list AFTER user info ready */
    DB.collection('posts').where('userId','==',uid).orderBy('createdAt','desc').limit(30).get().then(psnap=>{
      ...
      const visiblePosts=psnap.docs.map(doc=>({id:doc.id,...doc.data()})).filter(p=>!p.status||p.status==='uploading'||p.status==='failed');
      if(!visiblePosts.length){g.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round" style="font-size:44px;opacity:.3;display:block;margin-bottom:10px">grid_off</span><p>No posts yet</p></div>';return;}
      visiblePosts.forEach(p=>{
        const item=buildFbStylePostItem(p,userInfo,()=>openUserProfile(uid));
        g.appendChild(item);
      });
    }).catch(()=>{});
...
/* ══════════════════════════════════════
   YOUR POSTS — Facebook-style list
══════════════════════════════════════ */
function openYourPosts(){
  ...
  DB.collection('posts').where('userId','==',currentUser.uid).orderBy('createdAt','desc').limit(50).get().then(snap=>{
    ...
    if(!snap.size){body.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round" style="font-size:44px;opacity:.3;display:block;margin-bottom:10px">grid_off</span><p>No posts yet</p></div>';return;}
    snap.docs.forEach(doc=>{
      const p={id:doc.id,...doc.data()};
      const item=buildFbStylePostItem(p,userInfo,()=>openYourPosts());
      body.appendChild(item);
    });
  }).catch(err=>{setHTML('ypoBody','<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round">error</span><p>Failed to load</p><small>'+esc(err.message)+'</small></div>');});
}
...
function openLikedPosts(){
  ...
  Promise.all(ids.slice(0,20).map(id=>DB.collection('posts').doc(id).get())).then(async snaps=>{
    const body=$('ypoBody');if(!body) return;body.innerHTML='';body.style.padding='10px 0';
    const posts=snaps.filter(s=>s.exists&&!s.data().status).map(s=>({id:s.id,...s.data()}));
    if(!posts.length){body.innerHTML='<div style="text-align:center;padding:40px;color:var(--tm)"><span class="material-icons-round">favorite_border</span><p>No liked posts</p></div>';return;}
    for(const p of posts){
      let userInfo={name:p.userName||'User',photoURL:p.userPhoto||'',uid:p.userId};
      const item=buildFbStylePostItem(p,userInfo,()=>openLikedPosts());
      body.appendChild(item);
    }
...
/* ══════════════════════════════════════
   MESSAGES — show ALL users with their real photo,
   searchable, click to chat — FIXED
══════════════════════════════════════ */
let _allUsersCache=[];
...
async function loadAllUsersAndConversations(){
  ...
  setHTML('conversationsList', list.innerHTML + ''); /* no-op keep structure */

  try{
    /* 1. Get all conversations this user is part of */
    const convSnap=await DB.collection('conversations').where('participants','array-contains',currentUser.uid).orderBy('lastMessageAt','desc').limit(50).get();
    const convMap={};
    convSnap.docs.forEach(doc=>{
      const d=doc.data();
      const ou=(d.participants||[]).find(u=>u!==currentUser.uid)||'';
      if(ou) convMap[ou]={id:doc.id,...d};
    });

    /* 2. Get ALL registered users (every logged-in user shows here) */
    const usersSnap=await DB.collection('users').get();
    const allUsers=usersSnap.docs.map(d=>({id:d.id,...d.data()})).filter(u=>u.id!==currentUser.uid);
    _allUsersCache=allUsers;

    const empty2=$('conversationsEmpty');
    if(!allUsers.length){ if(empty2) empty2.style.display='flex'; return; }
    if(empty2) empty2.style.display='none';

    /* 3. Sort: users with existing conversation first (by recency), then rest alphabetically */
    const withConv=allUsers.filter(u=>convMap[u.id]).sort((a,b)=>{
      const ta=convMap[a.id].lastMessageAt&&convMap[a.id].lastMessageAt.toDate?convMap[a.id].lastMessageAt.toDate():new Date(0);
      const tb=convMap[b.id].lastMessageAt&&convMap[b.id].lastMessageAt.toDate?convMap[b.id].lastMessageAt.toDate():new Date(0);
      return tb-ta;
    });
    ...
    [...withConv,...withoutConv].forEach(u=>{
      renderConversationItem(u, convMap[u.id]);
    });
  }catch(e){
    console.error('Load users error:',e);
    toast('Could not load users: '+e.message,'error',4000);
  }
}

function renderConversationItem(u, conv){
  const list=$('conversationsList'); if(!list) return;
  ...
  item.dataset.name=(u.displayName||'User').toLowerCase();
  const name=u.displayName||'User';
  /* Real photo logo */
  const avHtml=u.photoURL&&u.photoURL.startsWith('http')
    ? `<img src="${esc(u.photoURL)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.outerHTML='${name[0].toUpperCase()}'"/>`
    : name[0].toUpperCase();
  item.innerHTML=`
    <div class="conv-avatar" style="background:${randomColor(u.id)}">${avHtml}</div>
    <div class="conv-info">
      <p class="conv-name">${esc(name)}</p>
      <p class="conv-last-msg${unread?' unread':''}">${lastMsg!==null?(lastMsg?esc(lastMsg):'<span style="color:var(--tm);font-style:italic">No messages yet</span>'):'<span style="color:var(--tm);font-style:italic">Tap to start chat</span>'}</p>
    </div>
    <div class="conv-meta">
...
/* Search — filters the rendered list AND searches all users if not yet loaded */
on('messagesSearch','input',e=>{
  ...
  /* If nothing matches in DOM but cache has more (in case of late load), try re-render */
  if(!anyVisible && _allUsersCache.length){
    const list=$('conversationsList');
    if(list){
      list.querySelectorAll('.conversation-item').forEach(i=>i.remove());
      const matches=_allUsersCache.filter(u=>(u.displayName||'').toLowerCase().includes(q));
      matches.forEach(u=>renderConversationItem(u,null));
    }
  }
});
on('newMessageBtn','click',()=>{ const si=$('messagesSearch'); if(si) si.focus(); toast('Search and tap a user to start chatting','info'); });
...
function _doStartChat(uid,name,photo,existingConvId){
  ...
  if(ha){
    if(photo&&photo.startsWith('http')) ha.innerHTML=`<img src="${esc(photo)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%" onerror="this.outerHTML='${(name||'U')[0].toUpperCase()}'"/>`;
...
/* Send message — image loads instantly like WhatsApp */
async function sendChatMessage(){
  ...
  /* Instant local preview for image before upload completes */
  if(file&&file.type.startsWith('image/')){
    const localUrl=URL.createObjectURL(file);
    const cm=$('chatMessages');
    if(cm){
      const tempEl=document.createElement('div');
      tempEl.className='chat-msg msg-sent';
      tempEl.style.opacity='.6';
      tempEl.innerHTML=`<div><div class="chat-msg-media"><img src="${localUrl}" style="border-radius:12px;max-width:220px;display:block;"/></div><p class="chat-msg-time">Sending...</p></div>`;
      cm.appendChild(tempEl);
      cm.scrollTop=cm.scrollHeight;
    }
  }
  ...
  try{
    ...
    if(file){
      ...
      if(mf) mf.value=''; setStyle('chatMediaPreview','display','none');
    }
...
on('publishFeaturedBtn','click',async()=>{
  ...
  /* Use link field — fallback to old title field id if link field missing */
  const linkInput=$('featuredLink')||$('featuredTitle');
  ...
  if(!link){toast('Link required — where should users go when they tap this ad?','error');return;}
  if(!thumbF){toast('Thumbnail/Image required','error');return;}
  ...
  try{
    ...
    if(type!=='image'&&mediaF) mediaUrl=await uploadFile(mediaF,'featured/'+(type==='video'?'videos':'audios')+'/',()=>{});
    await DB.collection('featured').add({title:'Sponsored',link,type,mediaType:type,thumbnailUrl:thumbUrl,mediaUrl,createdAt:TS()});
    toast('Sponsored ad published!','success');
...
setTimeout(updateLogos,600);

console.log('%cSayatX v7.0 Ready! Guest-only login.','color:#f97316;font-weight:900;font-size:16px');
