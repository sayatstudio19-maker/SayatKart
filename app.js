...
/* ============================================================
   ...
   Firebase project: sayaya-b71c0 (config embedded below — permanent, no
   more re-pasting on every new version). For push notifications in the
   background (app closed/minimised) you'll also need a
   firebase-messaging-sw.js service worker file — not part of this file,
   ask for it whenever you're ready.
   ============================================================ */
...
function hidePage(id) {
  ...
  if (!handlingPopstate) {
    // Sync history WITHOUT firing a second popstate (that was the bug —
    // history.back() here used to trigger the popstate handler again,
    // which then closed the page we'd just revealed too).
    history.replaceState({ sayatxTrap: true }, '', location.pathname + location.search);
  }
}
...
function defaultAvatarFor(id) {
  // Same id -> same placeholder face every time (pravatar's `u` param seeds it),
  // instead of a fresh random one on every render.
  return `https://i.pravatar.cc/150?u=${encodeURIComponent(id || 'sayatx-user')}`;
}
...
function playMessageSendSound() {
  try {
    ...
    [[880, 0], [1320, 0.06]].forEach(([freq, delay]) => {
...
function startCallRingSound() {
  ...
  ring();
  ringLoopTimer = setInterval(ring, 1800);
}
...
function playCallEndedSound() {
  try {
    const ctx = getAudioCtx();
    const now = ctx.currentTime;
    [493.88, 392.00, 293.66].forEach((freq, i) => {
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.type = 'sine';
      osc.frequency.value = freq;
...
async function emailLogin() {
  ...
  try {
    ...
    if (userDoc.exists) {
      state.profile = userDoc.data();
      await db.collection('users').doc(state.user.uid).update({ online: true, lastSeen: firebase.firestore.FieldValue.serverTimestamp() });
      enterApp();
    } else {
      goToAuthStep('profileSetupPage');
    }
    ...
    showToast(map[err.code] || `Could not log in (${err.code || err.message})`);
  } finally {
...
async function emailSignup() {
  ...
  const password = $('emailSignupPasswordInput').value;
  const confirm = $('emailSignupConfirmInput').value;
  ...
  try {
    ...
    console.error(err);
    const map = {
      'auth/email-already-in-use': 'An account with that email already exists — try Log In instead.',
      'auth/invalid-email': 'That email address looks invalid.',
      'auth/weak-password': 'Choose a stronger password (at least 6 characters).'
    };
    showToast(map[err.code] || `Could not sign up (${err.code || err.message})`);
  } finally {
...
async function sendPasswordReset() {
  ...
  if (!email) { showToast('Enter your account email'); return; }
  ...
  try {
    ...
    console.error(err);
    const map = { 'auth/user-not-found': 'No account with that email.', 'auth/invalid-email': 'That email address looks invalid.' };
    showToast(map[err.code] || `Could not send reset email (${err.code || err.message})`);
  } finally {
...
function loadHomeData() {
  ...
  state.chatsUnsub = db.collection('chats')
    ...
    .onSnapshot(async (snap) => {
      ...
      list.innerHTML = '';
      const visibleChats = snap.docs.filter(doc => !(doc.data().hiddenFor && doc.data().hiddenFor[state.user.uid]));
      const chattedUids = new Set();
      visibleChats.forEach(doc => (doc.data().members || []).forEach(uid => { if (uid !== state.user.uid) chattedUids.add(uid); }));
      visibleChats.forEach(doc => list.appendChild(renderChatListItem({ id: doc.id, ...doc.data() })));

      // Home doubles as discovery: show everyone else you haven't messaged yet below your chats.
      try {
        const usersSnap = await db.collection('users').limit(200).get();
        const others = usersSnap.docs.filter(d => d.id !== state.user.uid && !chattedUids.has(d.id));
        if (others.length) {
          const label = document.createElement('div');
          label.className = 'settings-section';
          label.textContent = visibleChats.length ? 'Other people on SayatX' : 'People on SayatX';
          list.appendChild(label);
          others.forEach(doc => {
            const u = doc.data();
            list.appendChild(renderChatListItem({
              otherUid: u.uid, name: u.name, photoURL: u.photoURL, online: u.online,
              lastMessage: `@${u.username || ''}`, isDiscoveryRow: true
            }));
          });
        }
      } catch (err) { console.error('load all users for home', err); }

      if (!list.children.length) {
        list.innerHTML = '<div class="empty-state"><i class="fa-regular fa-comments"></i><span>No one else has joined yet — invite a friend 👋</span></div>';
      }
...
async function searchUsers(query) {
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      row.innerHTML = `<img class="avatar avatar-md" src="${escapeHtml(u.photoURL || defaultAvatarFor(u.uid))}"><div class="row-body"><div class="row-name">${escapeHtml(u.name)}</div><span class="row-sub">@${escapeHtml(u.username)}</span></div>`;
...
/* ============================================================
   ...
   ============================================================ */
function renderChatListItem(chat, context) {
  context = context || 'home';
  const node = $('tmplChatListItem').content.cloneNode(true);
  const row = node.querySelector('.list-row');
  row.dataset.chatId = chat.id || '';
  row.querySelector('img').src = chat.photoURL || defaultAvatarFor(chat.otherUid || chat.id);
  ...
  if (chat.isDiscoveryRow) {
    row.addEventListener('click', async () => {
      const chatId = await getOrCreateChat(chat.otherUid);
      openChat(chatId, { id: chatId, uid: chat.otherUid, name: chat.name, photoURL: chat.photoURL, online: chat.online });
    });
    return row;
  }

  row.addEventListener('click', () => {
    if (context === 'locked') { state.lockTargetChatId = chat.id; state.pendingUnlockChatInfo = chat; state.lockModalMode = 'unlock'; showModal('lockChatModal'); $('lockModalTitle').textContent = 'Enter password to unlock'; }
    else openChat(chat.id, chat);
  });
  let pressTimer;
  const openMenuForContext = () => {
    if (context === 'home') { openChatCardMenu(chat.id); return; }
    state.tabItemChatId = chat.id;
    state.tabItemContext = context;
    const labelMap = { saved: 'Remove from Saved', locked: 'Unlock Chat', favourite: 'Remove from Favourite' };
    $('tabItemRemoveLabel').textContent = labelMap[context] || 'Remove';
    $('tabItemRemoveItem').querySelector('i').className = context === 'locked' ? 'fa-solid fa-lock-open' : 'fa-solid fa-xmark';
    showModal('tabItemMenuModal');
  };
  const startPress = () => { pressTimer = setTimeout(openMenuForContext, 3000); };
...
function loadYourChats(type) {
  ...
  if (type === 'groups') {
    db.collection('chats').where('members', 'array-contains', state.user.uid).where('isGroup', '==', true).get()
      .then(snap => {
        list.innerHTML = '';
        if (snap.empty) { list.innerHTML = '<div class="empty-state"><i class="fa-solid fa-user-group"></i><span>No groups yet — create one from Home</span></div>'; return; }
        snap.forEach(doc => list.appendChild(renderChatListItem({ id: doc.id, ...doc.data() }, 'home')));
  ...
  db.collection(rootCollection).doc(state.user.uid).collection(subCollection).get()
    .then(async snap => {
      list.innerHTML = '';
      if (snap.empty) {
        list.innerHTML = `<div class="empty-state"><i class="fa-regular fa-folder-open"></i><span>No ${escapeHtml(type)} chats yet</span></div>`;
        return;
      }
      for (const docSnap of snap.docs) {
        const chatDoc = await db.collection('chats').doc(docSnap.id).get();
        if (chatDoc.exists) list.appendChild(renderChatListItem({ id: chatDoc.id, ...chatDoc.data() }, type));
      }
...
function openProfileView(profile, isOwn) {
  ...
  $('profileViewAvatar').src = profile.photoURL || defaultAvatarFor(profile.uid);
  ...
  const grid = $('profileViewPostsGrid');
  grid.innerHTML = '<div class="empty-state"><i class="fa-solid fa-spinner fa-spin"></i><span>Loading posts...</span></div>';
  db.collection('posts').where('authorId', '==', profile.uid).orderBy('createdAt', 'desc').get()
    .then(snap => {
      $('profilePostCount').textContent = snap.size;
      grid.innerHTML = '';
      if (snap.empty) {
        grid.innerHTML = `<div class="empty-state"><i class="fa-regular fa-image"></i><span>${isOwn ? "You haven't" : "They haven't"} posted anything yet</span></div>`;
        return;
      }
      snap.forEach(doc => {
        const card = renderPostCard({ id: doc.id, ...doc.data() });
        if (isOwn) {
          const kebab = card.querySelector('.post-card-head .icon-btn');
          if (kebab) {
            const fresh = kebab.cloneNode(true); // drop the default report-wiring, own-post gets delete instead
            kebab.replaceWith(fresh);
            fresh.addEventListener('click', () => {
              showConfirm('Delete this post?', 'This cannot be undone.', async () => {
                await db.collection('posts').doc(doc.id).delete();
                openProfileView(state.profile, true);
              });
            });
            fresh.innerHTML = '<i class="fa-solid fa-trash" style="color:var(--alta);"></i>';
          }
        }
        grid.appendChild(card);
      });
    }).catch(err => console.error('load profile posts', err));
...
function loadBlockedUsers() {
  ...
  db.collection('blockedUsers').doc(state.user.uid).collection('users').get().then(async snap => {
    ...
    for (const docSnap of snap.docs) {
      ...
      const row = document.createElement('div');
      row.className = 'list-row';
      row.innerHTML = `<img class="avatar avatar-sm" src="${escapeHtml(u.photoURL || defaultAvatarFor(u.uid))}"><div class="row-body">${escapeHtml(u.name)}</div><button class="btn btn-outline" style="padding:8px 14px;font-size:12px;">Unblock</button>`;
...
function wireCoreEvents() {
  ...
  $('postsMenuYourPostsItem').addEventListener('click', () => { $('postsMenuDropdown').classList.add('hidden'); showPage('yourPostsPage'); loadYourPosts(); });
  $('postsMenuSettingsItem').addEventListener('click', () => { $('postsMenuDropdown').classList.add('hidden'); showPage('settingsPage'); });

  // Your Chats tabs (scoped — a generic .tabs .tab selector here would also
  // catch the login page's Phone/Email tabs, which need different behavior)
  document.querySelectorAll('#chatsScreen .tabs .tab').forEach(tab => {
    tab.addEventListener('click', () => {
      document.querySelectorAll('#chatsScreen .tabs .tab').forEach(t => t.classList.remove('active'));
  ...
  // Login page: Phone / Email method switch
  $('authMethodPhoneTab').addEventListener('click', () => {
    $('authMethodPhoneTab').classList.add('active');
    $('authMethodEmailTab').classList.remove('active');
    $('phoneLoginMethod').classList.remove('hidden');
    $('emailLoginMethod').classList.add('hidden');
  });
  $('authMethodEmailTab').addEventListener('click', () => {
    $('authMethodEmailTab').classList.add('active');
    $('authMethodPhoneTab').classList.remove('active');
    $('emailLoginMethod').classList.remove('hidden');
    $('phoneLoginMethod').classList.add('hidden');
  });
  $('goToSignupLink').addEventListener('click', () => { $('emailLoginView').classList.add('hidden'); $('emailSignupView').classList.remove('hidden'); });
  $('goToLoginLink').addEventListener('click', () => { $('emailSignupView').classList.add('hidden'); $('emailLoginView').classList.remove('hidden'); });
  $('forgotPasswordLink').addEventListener('click', () => { $('emailLoginView').classList.add('hidden'); $('forgotPasswordView').classList.remove('hidden'); });
  $('forgotPasswordBack').addEventListener('click', () => { $('forgotPasswordView').classList.add('hidden'); $('emailLoginView').classList.remove('hidden'); });
  ...
  $('profileViewMenuBtn').addEventListener('click', (e) => {
    e.stopPropagation();
    $('profileEditMenuItem').classList.toggle('hidden', !state.viewingOwnProfile);
    $('profileSettingsMenuItem').classList.toggle('hidden', !state.viewingOwnProfile);
    $('profileOtherDivider').classList.toggle('hidden', state.viewingOwnProfile);
    $('profileReportMenuItem').classList.toggle('hidden', state.viewingOwnProfile);
    $('profileBlockMenuItem').classList.toggle('hidden', state.viewingOwnProfile);
    toggleDropdown('profileViewMenuDropdown', $('profileViewMenuBtn'));
  });
  $('profileEditMenuItem').addEventListener('click', () => {
    $('profileViewMenuDropdown').classList.add('hidden');
    hidePage('profileViewPage'); showPage('editProfilePage'); populateEditProfile();
  });
  $('profileShareMenuItem').addEventListener('click', () => {
    $('profileViewMenuDropdown').classList.add('hidden');
    navigator.clipboard.writeText(`https://sayatx.vercel.app/u/${state.viewingProfileUid}`);
    showToast('Profile link copied');
  });
  $('profileSettingsMenuItem').addEventListener('click', () => {
    $('profileViewMenuDropdown').classList.add('hidden');
    hidePage('profileViewPage'); showPage('settingsPage');
  });
  $('profileReportMenuItem').addEventListener('click', () => {
    $('profileViewMenuDropdown').classList.add('hidden');
    openReport('user', state.viewingProfileUid);
  });
  $('profileBlockMenuItem').addEventListener('click', async () => {
    $('profileViewMenuDropdown').classList.add('hidden');
    await db.collection('blockedUsers').doc(state.user.uid).collection('users').doc(state.viewingProfileUid).set({ blockedAt: firebase.firestore.FieldValue.serverTimestamp() });
  ...
  // (profileViewPostsBtn removed — posts now render directly on the profile page,
  // see #profileViewPostsGrid; will wire that up when JS work resumes)
  ...
  $('notifSettingsBtn').addEventListener('click', () => { showPage('notificationSettingsPage'); loadNotificationSettings(); });
  $('notifSettingsBack').addEventListener('click', () => hidePage('notificationSettingsPage'));
  ['notifToggleMessage', 'notifToggleGroup', 'notifToggleLike', 'notifToggleComment'].forEach(id => {
...
/* ============================================================
   ...
   ============================================================ */
async function openChat(chatId, chatInfo) {
  state.currentChatId = chatId;
  state.currentChatUser = chatInfo;
  $('chatHeaderName').textContent = chatInfo.name || 'Chat';
  $('chatHeaderAvatar').src = chatInfo.photoURL || defaultAvatarFor(chatInfo.uid || chatInfo.id);
  ...
  try {
    const blockedSnap = await db.collection('blockedUsers').doc(state.user.uid).collection('users').get();
    state.blockedSenders = new Set(blockedSnap.docs.map(d => d.id));
  } catch (err) { state.blockedSenders = new Set(); }
  state.groupMemberNames = null;
  if (chatInfo.isGroup && Array.isArray(chatInfo.members)) {
    try {
      const memberDocs = await Promise.all(chatInfo.members.map(uid => db.collection('users').doc(uid).get()));
      state.groupMemberNames = {};
      memberDocs.forEach(d => { if (d.exists) state.groupMemberNames[d.id] = { name: d.data().name, photoURL: d.data().photoURL }; });
    } catch (err) { console.error('load group member names', err); }
  }
  loadMessages(chatId);
...
const notifTypeToSettingKey = { message: 'message', group: 'group', like: 'like', comment: 'comment' };
async function createNotification(toUid, type, text, refId) {
  ...
  try {
    ...
    if (settingKey) {
      const settingsDoc = await db.collection('settings').doc(toUid).get();
      const prefs = settingsDoc.exists ? settingsDoc.data().notifPrefs : null;
      if (prefs && prefs[settingKey] === false) return; // recipient turned this type off
    }
...
async function loadNotificationSettings() {
  try {
    const doc = await db.collection('settings').doc(state.user.uid).get();
    const prefs = (doc.exists && doc.data().notifPrefs) || {};
    $('notifToggleMessage').classList.toggle('on', prefs.message !== false);
    $('notifToggleGroup').classList.toggle('on', prefs.group !== false);
    $('notifToggleLike').classList.toggle('on', prefs.like !== false);
    $('notifToggleComment').classList.toggle('on', prefs.comment !== false);
  } catch (err) { console.error('load notification settings', err); }
}
function saveNotificationSettings() {
  const prefs = {
    message: $('notifToggleMessage').classList.contains('on'),
    group: $('notifToggleGroup').classList.contains('on'),
    like: $('notifToggleLike').classList.contains('on'),
    comment: $('notifToggleComment').classList.contains('on')
  };
  db.collection('settings').doc(state.user.uid).set({ notifPrefs: prefs }, { merge: true }).catch(err => console.error('save notification settings', err));
}
...
function renderMessage(msg) {
  ...
  if (msg.type === 'call') {
    ...
    const icon = msg.outcome === 'missed' ? 'fa-phone-slash' : 'fa-phone';
    const label = msg.outcome === 'missed' ? 'Missed call' : `Call ended · ${mins}:${String(secs).padStart(2, '0')}`;
    bubble.querySelector('.msg-text').outerHTML = `<i class="fa-solid ${icon}" style="margin-right:6px;${msg.outcome === 'missed' ? 'color:var(--alta);' : ''}"></i>${escapeHtml(label)}`;
  } else if (msg.deleted) {
  ...
  if (msg.replyTo) {
    ...
    quote.style.cssText = 'border-left:2px solid currentColor;opacity:.75;padding:2px 8px;margin-bottom:5px;font-size:12px;';
    quote.innerHTML = `<b>${escapeHtml(msg.replyTo.senderName || '')}</b><br>${escapeHtml((msg.replyTo.preview || '').slice(0, 60))}`;
  ...
  } else if (state.currentChatUser && state.currentChatUser.isGroup && msg.type !== 'call' && state.groupMemberNames) {
    const senderInfo = state.groupMemberNames[msg.senderId];
    if (senderInfo) {
      const nameEl = document.createElement('div');
      nameEl.textContent = senderInfo.name;
      nameEl.style.cssText = 'font-size:12px;font-weight:800;color:var(--terracotta);margin-bottom:3px;';
  ...
  state.messagesUnsub = db.collection('chats').doc(chatId).collection('messages')
    ...
    .onSnapshot(snap => {
      ...
      snap.forEach(doc => {
        ...
        if (msg.senderId !== state.user.uid && state.blockedSenders && state.blockedSenders.has(msg.senderId)) return; // blocked: never reaches me
        if (msg.senderId !== state.user.uid && !msg.seen) toMarkSeen.push(doc.ref);
      ...
      if (toMarkSeen.length) {
        const batch = db.batch();
        toMarkSeen.forEach(ref => batch.update(ref, { delivered: true, seen: true }));
        batch.commit().catch(err => console.error('mark seen', err));
      }
...
async function sendTextMessage() {
  ...
  if (state.replyingTo) { data.replyTo = state.replyingTo; clearReplyPreview(); }
...
function showReplyPreview(senderName, preview) {
  clearReplyPreview();
  const bar = document.createElement('div');
  bar.id = 'replyPreviewBar';
  bar.style.cssText = 'display:flex;align-items:center;gap:10px;padding:8px 16px;border-top:1px solid var(--hairline);background:var(--ink-raised-2);';
  bar.innerHTML = `<div style="flex:1;border-left:2px solid var(--terracotta);padding-left:8px;"><b style="font-size:12px;color:var(--terracotta);">${escapeHtml(senderName)}</b><div style="font-size:12px;color:var(--paper-dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(preview)}</div></div><button id="cancelReplyBtn" class="icon-btn"><i class="fa-solid fa-xmark"></i></button>`;
...
async function sendMediaMessage(file, type, caption) {
  ...
  try {
    ...
    const data = { type, senderId: state.user.uid, mediaURL: url, caption: caption || '', createdAt: firebase.firestore.FieldValue.serverTimestamp(), delivered: false, seen: false };
...
async function openForwardModal() {
  ...
  list.innerHTML = '<div style="padding:20px;text-align:center;color:var(--paper-faint);font-size:13px;">Loading chats...</div>';
  showModal('forwardMessageModal');
  try {
    const snap = await db.collection('chats').where('members', 'array-contains', state.user.uid).get();
    list.innerHTML = '';
    if (snap.empty) { list.innerHTML = '<div class="empty-state"><i class="fa-regular fa-comments"></i><span>No chats to forward to yet</span></div>'; return; }
    snap.forEach(doc => {
      const c = doc.data();
      const row = document.createElement('div');
      row.className = 'list-row';
      row.dataset.chatId = doc.id;
      row.innerHTML = `<img class="avatar avatar-sm" src="${escapeHtml(c.photoURL || defaultAvatarFor(doc.id))}"><div class="row-body">${escapeHtml(c.name || 'Chat')}</div><input type="checkbox">`;
      row.addEventListener('click', (e) => { if (e.target.tagName !== 'INPUT') row.querySelector('input').checked = !row.querySelector('input').checked; });
      list.appendChild(row);
    });
  } catch (err) { console.error('load forward chats', err); }
}
async function forwardSelectedMessage() {
  const sel = state.selectedMessageEl;
  if (!sel) return;
  const checked = document.querySelectorAll('#forwardChatList input[type=checkbox]:checked');
  if (!checked.length) { showToast('Choose at least one chat'); return; }
  const msg = sel.msg;
  const targets = Array.from(checked).map(cb => cb.closest('.list-row').dataset.chatId);
  try {
    for (const chatId of targets) {
      const data = {
        type: msg.type, senderId: state.user.uid,
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
        delivered: false, seen: false, forwarded: true
      };
      if (msg.text) data.text = msg.text;
      if (msg.mediaURL) data.mediaURL = msg.mediaURL;
      if (msg.caption) data.caption = msg.caption;
      if (msg.fileName) data.fileName = msg.fileName;
      if (msg.fileSize) data.fileSize = msg.fileSize;
      if (msg.duration) data.duration = msg.duration;
      await db.collection('chats').doc(chatId).collection('messages').add(data);
      await db.collection('chats').doc(chatId).update({ lastMessage: msg.text || 'Forwarded message', updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
    }
    showToast(`Forwarded to ${targets.length} chat${targets.length > 1 ? 's' : ''}`);
    hideModal('forwardMessageModal');
  } catch (err) { console.error('forward message', err); showToast('Could not forward'); }
}
...
async function confirmLockChat() {
  ...
  const hasPasswordSet = lockDoc.exists && lockDoc.data().passwordHash;

  if (state.lockModalMode === 'unlock') {
    if (!hasPasswordSet || lockDoc.data().passwordHash !== hash) { showToast('Incorrect password'); return; }
    hideModal('lockChatModal');
    $('lockPasswordInput').value = '';
    if (state.pendingUnlockChatInfo) openChat(state.lockTargetChatId, state.pendingUnlockChatInfo);
    return;
  }

  // mode === 'lock' (setting/confirming lock on a chat)
  if (hasPasswordSet) {
...
async function loadGroupMemberPicker() {
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      row.innerHTML = `<img class="avatar avatar-sm" src="${escapeHtml(u.photoURL || defaultAvatarFor(u.uid))}"><div class="row-body">${escapeHtml(u.name)}<div class="row-sub">@${escapeHtml(u.username)}</div></div><input type="checkbox">`;
...
function openGroupInfo(groupId) {
  db.collection('groups').doc(groupId).get().then(async (doc) => {
    ...
    memberDocs.forEach(mDoc => {
      ...
      const row = document.createElement('div');
      row.className = 'list-row';
      row.innerHTML = `<img class="avatar avatar-sm" src="${escapeHtml(m.photoURL || defaultAvatarFor(m.uid))}"><div class="row-body">${escapeHtml(m.name)}</div>${isAdmin ? '<span class="row-sub" style="color:var(--terracotta);font-weight:700;">Admin</span>' : ''}`;
...
function renderPostCard(post) {
  ...
  node.querySelector('.post-card-head img').src = post.authorPhoto || defaultAvatarFor(post.authorId);
...
async function openComments(postId) {
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      const row = document.createElement('div');
      row.className = 'list-row';
      row.innerHTML = `<img class="avatar avatar-sm" src="${escapeHtml(c.authorPhoto || defaultAvatarFor(c.authorId))}"><div class="row-body"><b style="font-size:13px;">${escapeHtml(c.authorName)}</b><div style="font-size:13px;color:var(--paper-dim);">${escapeHtml(c.text)}</div></div>`;
      list.appendChild(row);
    });
...
function openShareModal(postId) {
  ...
  $('shareLinkInput').value = `https://sayatx.vercel.app/p/${postId}`;
...
function showCallScreen(partner, statusText, type) {
  const photo = (partner && partner.photoURL) || defaultAvatarFor(partner && (partner.uid || partner.id));
  $('callPartnerAvatar').src = photo;
  ...
  $('callingScreen').classList.toggle('audio-mode', type !== 'video');
  $('audioCallBackdrop').style.backgroundImage = `url(${photo})`;
}
...
function startCallTimer() {
  ...
  callTimerInterval = setInterval(() => {
    ...
    const m = Math.floor(callElapsedSeconds / 60), s = callElapsedSeconds % 60;
    $('callStatus').textContent = `${m}:${String(s).padStart(2, '0')}`;
  }, 1000);
}
function stopCallTimer() { clearInterval(callTimerInterval); }
async function logCallMessage(callRef, outcome) {
  try {
    const doc = await callRef.get();
    if (!doc.exists) return;
    const data = doc.data();
    if (!data.chatId) return;
    await db.collection('chats').doc(data.chatId).collection('messages').add({
      type: 'call', callType: data.type || 'audio', outcome, durationSec: callElapsedSeconds,
      senderId: data.callerId, createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      delivered: true, seen: true
    });
    const label = outcome === 'missed' ? '📞 Missed call' : '📞 Call ended';
    await db.collection('chats').doc(data.chatId).update({ lastMessage: label, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
    if (outcome === 'missed') {
      const callerName = (state.profile && data.callerId === state.user.uid) ? state.profile.name : (state.currentChatUser && state.currentChatUser.name);
      createNotification(data.receiverId, 'message', `<b>${escapeHtml(callerName || 'Someone')}</b> called you`, data.chatId);
    }
  } catch (err) { console.error('log call message', err); }
}
...
async function startCall(type) {
  if (!state.currentChatUser || !state.user || !state.currentChatId) return;
  ...
  showCallScreen(state.currentChatUser, 'Calling...', type);
...
async function rejectOrEndCall() {
  ...
  if (ref) {
    await ref.update({ status: 'ended' }).catch(() => {});
    await logCallMessage(ref, callWasConnected ? 'ended' : 'missed');
  }
  endCallLocal();
}

function endCallLocal() {
  const wasShowing = !$('callingScreen').classList.contains('hidden');
    ...
    .onSnapshot(snap => {
      snap.docChanges().forEach(change => {
        if (change.type === 'added') {
          ...
          currentCallDocRef = db.collection('calls').doc(change.doc.id); // lets Reject work pre-answer
          db.collection('users').doc(data.callerId).get().then(doc => {
            const caller = doc.exists ? doc.data() : { name: 'Unknown' };
            showCallScreen(caller, `Incoming ${data.type} call...`, data.type);
            startCallRingSound();
...
function wireChatPostEvents() {
  ...
  $('lockChatMenuItem').addEventListener('click', () => { state.lockTargetChatId = state.currentChatId; state.lockModalMode = 'lock'; $('lockModalTitle').textContent = 'Your Favourite Person?'; $('chatMenuDropdown').classList.add('hidden'); showModal('lockChatModal'); });
  ...
  $('replyMsgItem').addEventListener('click', () => {
    $('messageContextMenu').classList.add('hidden');
    if (!state.selectedMessageEl) return;
    const msg = state.selectedMessageEl.msg;
    const senderName = msg.senderId === state.user.uid ? 'You' : ((state.groupMemberNames && state.groupMemberNames[msg.senderId] && state.groupMemberNames[msg.senderId].name) || state.currentChatUser.name);
    const preview = msg.text || (msg.type === 'image' ? '📷 Photo' : msg.type === 'video' ? '🎬 Video' : msg.type === 'voice' ? '🎤 Voice message' : msg.type === 'file' ? '📎 File' : '');
    state.replyingTo = { id: msg.id, senderName, preview };
    showReplyPreview(senderName, preview);
  });
  $('forwardMsgItem').addEventListener('click', () => { $('messageContextMenu').classList.add('hidden'); openForwardModal(); });
  document.querySelector('#forwardMessageModal .btn-primary').addEventListener('click', forwardSelectedMessage);
  ...
  document.querySelectorAll('#reactionPicker span').forEach(s => s.addEventListener('click', async () => {
    $('reactionPicker').classList.add('hidden');
    if (!state.selectedMessageEl || !state.currentChatId) return;
    try {
      await db.collection('chats').doc(state.currentChatId).collection('messages').doc(state.selectedMessageEl.msg.id)
        .update({ [`reactions.${state.user.uid}`]: s.textContent });
  ...
  $('cardFavouriteChatItem').addEventListener('click', async () => {
    hideModal('chatCardMenuModal');
    await db.collection('savedChats').doc(state.user.uid).collection('favourites').doc(state.cardMenuChatId).set({ addedAt: firebase.firestore.FieldValue.serverTimestamp() });
    showToast('Added to favourites');
  });
  $('tabItemOpenChatItem').addEventListener('click', () => {
    hideModal('tabItemMenuModal');
    if (state.tabItemContext === 'locked') { /* keep it gated — long-press again if you want to unlock */ showToast('This chat is locked'); return; }
    db.collection('chats').doc(state.tabItemChatId).get().then(doc => { if (doc.exists) openChat(doc.id, doc.data()); });
  });
  $('tabItemRemoveItem').addEventListener('click', async () => {
    hideModal('tabItemMenuModal');
    const chatId = state.tabItemChatId, ctx = state.tabItemContext;
    if (ctx === 'saved') await db.collection('savedChats').doc(state.user.uid).collection('chats').doc(chatId).delete();
    else if (ctx === 'favourite') await db.collection('savedChats').doc(state.user.uid).collection('favourites').doc(chatId).delete();
    else if (ctx === 'locked') await db.collection('lockedChats').doc(state.user.uid).collection('chats').doc(chatId).delete();
    showToast(ctx === 'locked' ? 'Chat unlocked' : 'Removed');
    loadYourChats(ctx);
  });
  $('cardLockChatItem').addEventListener('click', () => { hideModal('chatCardMenuModal'); state.lockTargetChatId = state.cardMenuChatId; state.lockModalMode = 'lock'; $('lockModalTitle').textContent = 'Your Favourite Person?'; showModal('lockChatModal'); });
  ...
  $('shareToWhatsApp').addEventListener('click', () => {
    window.open(`https://wa.me/?text=${encodeURIComponent($('shareLinkInput').value)}`, '_blank');
    if (activeSharePostId) db.collection('posts').doc(activeSharePostId).update({ shareCount: firebase.firestore.FieldValue.increment(1) }).catch(() => {});
  });
  $('shareToTelegram').addEventListener('click', () => {
    window.open(`https://t.me/share/url?url=${encodeURIComponent($('shareLinkInput').value)}`, '_blank');
    if (activeSharePostId) db.collection('posts').doc(activeSharePostId).update({ shareCount: firebase.firestore.FieldValue.increment(1) }).catch(() => {});
  });
  $('shareToTwitter').addEventListener('click', () => {
    window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent($('shareLinkInput').value)}`, '_blank');
    if (activeSharePostId) db.collection('posts').doc(activeSharePostId).update({ shareCount: firebase.firestore.FieldValue.increment(1) }).catch(() => {});
  });
  $('shareToMore').addEventListener('click', async () => {
    if (navigator.share) {
      try { await navigator.share({ url: $('shareLinkInput').value }); } catch (e) {}
    } else {
      copyShareLink();
    }
  });
...
window.addEventListener('popstate', () => {
  ...
  try {
    ...
    // Nothing open — true root. Let this back press through normally (exits/navigates away).
  } finally {
