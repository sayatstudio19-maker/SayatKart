...
/* ---------- 0. VISIBLE ERROR BANNER ----------
   If anything in this file throws — or the Firebase SDK fails to
   load (blocked network, ad-blocker, etc.) — show it on screen
   instead of failing silently. This is what used to look like
   "nothing happens, field just clears" (a plain page reload from
   the browser's default form submission, because our JS never
   got the chance to run e.preventDefault()). */
...
function showFatalError(message) {
  ...
  if (!banner) {
    ...
    banner.style.cssText = `
      ...
      background:#FF5C5C; color:#fff; font-size:13px; padding:10px 16px;
...
window.addEventListener('error', (e) => {
  showFatalError(`App error: ${e.message}`);
});
window.addEventListener('unhandledrejection', (e) => {
  showFatalError(`App error: ${e.reason && e.reason.message ? e.reason.message : e.reason}`);
});

if (typeof firebase === 'undefined') {
  showFatalError('Firebase failed to load — check your internet connection or ad-blocker, then reload the page.');
  throw new Error('Firebase SDK not loaded');
}
if (!firebase.auth || !firebase.firestore || !firebase.storage) {
  showFatalError('Firebase Auth/Firestore/Storage scripts failed to load — check your internet connection, then reload the page.');
  throw new Error('Firebase compat modules not loaded');
}
...
$('#step-mobile').addEventListener('submit', async (e) => {
  ...
  if (!/^[6-9]\d{9}$/.test(digits)) {
    hint.textContent = 'Enter a valid 10-digit Indian mobile number.';
    return;
  }

  fullPhoneNumber = `${INDIA_CODE}${digits}`;
